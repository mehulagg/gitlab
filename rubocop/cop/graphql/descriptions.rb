      ...
      class Descriptions < RuboCop::Cop::Cop
        ...
        def autocorrect(node)
          lambda do |corrector|
            ...
            if description.location.is_a?(Parser::Source::Map::Heredoc)
              heredoc_source = description.location.heredoc_body.source
              adjust = heredoc_source.index(/\s+\Z/) - heredoc_source.length
              before_end_quote = description.location.heredoc_body.adjust(end_pos: adjust)
            else
              before_end_quote = description.source_range.adjust(end_pos: -1)
            end

            corrector.insert_after(before_end_quote, '.')
          end
        end
