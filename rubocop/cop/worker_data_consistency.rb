  ...
  module Cop
    ...
    class WorkerDataConsistency < RuboCop::Cop::Cop
      ...
      MSG = <<~MSG
        ...
        worker always hitting the primary database node for both reads and writes, which curbs
        scalability.

        Some guidelines:
        
        1. If your worker mostly writes or reads its own writes, use mode :always. TRY TO AVOID THIS.
        2. If your worker performs mostly reads and can suffer small delays, use mode :delayed.
        3. If your worker performs mostly reads but should not suffer any delays, use mode :sticky.
