...
ActiveRecord::Schema.define(version: 2021_07_06_120643) do
  ...
  create_table "upload_registry", force: :cascade do |t|
    ...
    t.string "verification_failure", limit: 255
    t.string "last_sync_failure", limit: 255
    t.index ["retry_at"], name: "index_upload_registry_on_retry_at"
    t.index ["state"], name: "index_upload_registry_on_state"
    t.index ["upload_id"], name: "index_upload_registry_on_upload_id", unique: true
    t.index ["verification_retry_at"], name: "upload_registry_failed_verification", order: "NULLS FIRST", where: "((state = 2) AND (verification_state = 3))"
    t.index ["verification_state"], name: "upload_registry_needs_verification", where: "((state = 2) AND (verification_state = ANY (ARRAY[0, 3])))"
    t.index ["verified_at"], name: "upload_registry_pending_verification", order: "NULLS FIRST", where: "((state = 2) AND (verification_state = 0))"
  end
