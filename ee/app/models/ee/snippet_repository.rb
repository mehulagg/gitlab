  ...
  module SnippetRepository
    ...
    class_methods do
      ...
      def snippet_repositories_for_selected_namespaces(primary_key_in)
        personal_snippets = self.joins(:snippet).where(snippet: ::Snippet.only_personal_snippets).primary_key_in(primary_key_in)

        project_snippets = self.joins(:snippet).where(
        ...
        # it increases the query time by a factor of 10.
