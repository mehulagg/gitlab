...
module EE
  ...
  module Group
    ...
    def releases_count
      ::Release.joins(:project).
        where(projects: {namespace_id: self_and_descendants.select(:id)})
        .count
    end
    ...
    def releases_percentage
      calculate_sql = <<~SQL
      (
        COUNT(*) FILTER (WHERE EXISTS (SELECT 1 FROM releases WHERE releases.project_id = projects.id)) * 100.0 / COUNT(*)
