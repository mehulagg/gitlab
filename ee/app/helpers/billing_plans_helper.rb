...
module BillingPlansHelper
  ...
  def subscription_plan_data_attributes(namespace, plan)
...

    {
      ...
      plan_name: plan&.name,
      free_personal_namespace: free_personal_namespace?(namespace.id)
    }
  end
  ...
  def show_plans?(namespace)
    !free_personal_namespace?(namespace.id) && (namespace.trial_active? || !(namespace.gold_plan? || namespace.ultimate_plan?))
  end
  ...
  def free_personal_namespace?(namespace_id)
    # TODO - implement this
    false
  end
end
