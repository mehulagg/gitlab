...
module Iterations
  module Cadences
    class CreateService
      ...
      def execute
        return ::ServiceResponse.error(message: _('Operation not allowed'), http_status: 403) unless can_create_iterations_cadence?
      ...
      def can_create_iterations_cadence?
        group.iterations_cadences_enabled? &&
          group.feature_available?(:iterations) &&
          can?(current_user, :create_iterations_cadence, group)
      end
    end
  end
end
