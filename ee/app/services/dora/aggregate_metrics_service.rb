...
module Dora
  class AggregateMetricsService < ::BaseContainerService
    ...
    DEFAULT_AFTER = 3.months.ago.to_date
    ...
    def execute
      ...
      data = ::Dora::DailyMetrics
        .in_range_of(environments, after, before)
    ...
    def validate
      ...
      unless after < before
        return error(_('The after date must be ealier than the before date.'), :bad_request)
      end

      unless project? || group?
        return error(_('Unsupported container level.'), :bad_request)
      end
