...
module Security
  class DependencyListService
    ...
    def sort(collection)
      ...
      when 'severity'
        collection = sort_dependency_vulnerabilities_by_severity(collection)
        collection = sort_dependencies_by_severity(collection)
      else
    ...
    def sort_dependency_vulnerabilities_by_severity(collection)
      collection.each do |c|
        c[:vulnerabilities].sort! do |v1, v2|
          compare_severity_levels(v1[:severity], v2[:severity])
        end
      end
    end
