...
import { DEFAULT_MR_TITLE, DEFAULT_PROJECT_NAME } from './constants';
...
const assignSecurityPolicyProject = async (projectPath) => {
  const defaultProjectPath = projectPath + DEFAULT_PROJECT_NAME;
...
export const savePolicy = async ({ assignedPolicyProject, projectPath, yamlEditorValue }) => {
  ...
  if (!currentAssignedPolicyProject.path) {
  ...
  const newPolicyCommitBranch = await updatePolicy({
    projectPath: currentAssignedPolicyProject.path,
    yamlEditorValue,
  });
