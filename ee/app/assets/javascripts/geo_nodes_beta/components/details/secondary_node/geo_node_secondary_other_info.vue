...
<template>
  <gl-card>
    ...
    <div class="gl-display-flex gl-flex-direction-column gl-mb-5">
      ...
      <span class="gl-font-weight-bold gl-mt-2">
        <gl-sprintf v-if="node.lastEventId" :message="$options.i18n.lastEvent">
          ...
          <template #timeAgo>
            <time-ago data-testid="last-event" :time="lastEventTimestamp" />
          </template>
        </gl-sprintf>
    ...
    <div class="gl-display-flex gl-flex-direction-column gl-mb-5">
      ...
      <span class="gl-font-weight-bold gl-mt-2">
        <gl-sprintf v-if="node.lastCursorEventId" :message="$options.i18n.lastEvent">
          <template #eventId>
            {{ node.lastCursorEventId }}
          </template>
          <template #timeAgo>
            <time-ago data-testid="last-cursor-event" :time="lastCursorEventTimestamp" />
          </template>
        </gl-sprintf>
        <span v-else>{{ $options.i18n.unknown }}</span>
      </span>
