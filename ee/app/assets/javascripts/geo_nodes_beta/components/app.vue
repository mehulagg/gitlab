...
<template>
  <section>
    ...
    <div v-if="!isLoading && !noNodes">
      <h4 class="gl-font-lg gl-my-5">{{ $options.i18n.primarySite }}</h4>
      <geo-nodes v-for="node in primaryNodes" :key="node.id" :node="node" data-testid="primary-nodes" />
      <h4 class="gl-font-lg gl-my-5">{{ $options.i18n.secondarySite }}</h4>
      <geo-nodes v-for="node in secondaryNodes" :key="node.id" :node="node" data-testid="secondary-nodes" />
    </div>
    <geo-nodes-empty-state v-if="!isLoading && noNodes" :svg-path="geoNodesEmptyStateSvg" />
    <gl-modal
