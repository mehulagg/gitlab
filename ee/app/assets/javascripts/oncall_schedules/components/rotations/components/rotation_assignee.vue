...
export default {
  ...
  computed: {
    ...
    startsAt() {
      return sprintf(__('Starts: %{startsAt}'), { startsAt: formatDate(this.rotationAssigneeStartsAt, 'mmmm d, yyyy, hh:mm') });
    },
    endsAt() {
      return sprintf(__('Ends: %{endsAt}'), { endsAt: formatDate(this.rotationAssigneeEndsAt, 'mmmm d, yyyy, hh:mm') });
    }
  },
};
</script>
