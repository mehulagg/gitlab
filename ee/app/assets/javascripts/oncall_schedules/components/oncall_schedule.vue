...
import {formatDate, nWeeksBefore, nWeeksAfter, nDaysBefore, nDaysAfter} from '~/lib/utils/datetime_utility';
    ...
    scheduleRange() {
      switch (this.presetType) {
        ...
        case PRESET_TYPES.WEEKS: {
          ...
          return `${formatDate(this.timeframe[0], 'mmmm d')} - ${formatDate(lastDayOfTimeframe, 'mmmm d, yyyy')}`;
        }
        default:
          return '';
      }
  ...
  <div>
    <gl-card class="gl-mt-5" header-class="gl-py-3">
      ...
      <div class="gl-w-full gl-display-flex gl-align-items-center gl-pb-3">
        <gl-button-group>
          <gl-button icon="chevron-left"  @click="updateToViewPreviousTimeframe" :disabled="isLoading"/>
          <gl-button icon="chevron-right" @click="updateToViewNextTimeframe" :disabled="isLoading"/>
        </gl-button-group>
