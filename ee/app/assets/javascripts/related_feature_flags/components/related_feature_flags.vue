  ...
  mounted() {
    if (this.endpoint) {
      axios
        ...
        .catch((error) =>
          createFlash({
            message: __('There was an error loading related feature flags'),
            error,
          }),
        )
        .finally(() => {
...
<template>
  ...
  >
    ...
    >
      <div
        data-testid="referenced-feature-flags-header"
        class="card-title gl-my-0 gl-display-flex gl-align-items-center gl-w-full gl-relative"
      >
        <gl-link
          id="user-content-related-feature-flags"
          class="anchor gl-text-decoration-none gl-absolute"
          href="#related-feature-flags"
          aria-hidden="true"
        />
        <h3 class="h5 gl-my-0 gl-mr-2">
          {{ __('Related feature flags') }}
        </h3>
