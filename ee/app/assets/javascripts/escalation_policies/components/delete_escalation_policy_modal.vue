...
import destroyEscalationPolicyMutation from '../graphql/mutations/destroy_escalatiion_policy.graphql';
...
export const i18n = {
  ...
  deleteEscalationPolicyMessage: s__(
    'EscalationPolicies|Are you sure you want to delete the "%{deleteEscalationPolicy}"? This action cannot be undone.',
  ),
};
...
<template>
  <gl-modal
    ...
    :modal-id="modalId"
    size="sm"
    :data-testid="`delete-escalationPolicy-modal-${escalationPolicy.id}`"
  ...
  >
    ...
    <gl-sprintf :message="$options.i18n.deleteEscalationPolicyMessage">
      <template #deleteEscalationPolicy>{{ escalationPolicy.name }}</template>
    </gl-sprintf>
  </gl-modal>
</template>
