...
import { cleanLeadingSeparator, joinPaths, stripFilename } from '~/lib/utils/url_utility';
import {
    ...
    token: {
      ...
      handler() {
        if (!this.validationPathInputTouched) {
          this.validationPath = joinPaths(stripFilename(this.path), this.textFileName);
        }
      },
    },
  },
