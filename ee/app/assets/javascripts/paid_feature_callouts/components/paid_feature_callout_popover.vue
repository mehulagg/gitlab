...
export default {
  ...
  methods: {
    ...
    onShown() {
      const { action, ...options } = this.tracking.events.popoverShown;
      this.track(action, options);
    },
    onUpgradeBtnClick() {
      const { action, ...options } = this.tracking.events.upgradeBtnClick;
      this.track(action, options);
    },
    onCompareBtnClick() {
      const { action, ...options } = this.tracking.events.compareBtnClick;
      this.track(action, options);
    },
  },
};
