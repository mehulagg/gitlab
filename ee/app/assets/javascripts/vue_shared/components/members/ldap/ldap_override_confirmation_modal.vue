...
export default {
  ...
  i18n: {
    modalTitle: __('Edit permissions'),
    modalBody: __(
      '%{userName} is currently an LDAP user. Editing their permissions will override the settings from the LDAP group sync.',
    ),
  },
    ...
    handlePrimary() {
      ...
      this.updateLdapOverride({ memberId: this.memberToOverride.id, override: true })
        .then(() => {
          ...
          this.$toast.show(s__('Members|LDAP override enabled.'));
        })
        .catch(() => {
          this.hideLdapOverrideConfirmationModal();
          this.busy = false;
