...
export default {
  ...
  data() {
    const {
      ...
      excludedUrls = [],
      auth = {},
      targetType = targetTypes.website.value,
    } = this.siteProfile || {};
    ...
    return {
      ...
      errorMessage: '',
      errors: [],
      targetTypesOptions: Object.values(targetTypes),
    };
  },
  computed: {
    isEdit() {
  ...
  methods: {
    onSubmit() {
      ...
      const { profileName, targetUrl, targetType, ...additionalFields } = serializeFormObject(
        this.form.fields,
      );
