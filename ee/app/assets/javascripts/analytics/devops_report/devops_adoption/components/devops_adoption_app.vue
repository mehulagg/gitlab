...
import {
  DA_GROUPS_QUERY_ERROR,
  DA_ENABLED_NAMESPACE_QUERY_ERROR,
  DA_ENABLE_NAMESPACE_MUTATION_ERROR,
  DATE_TIME_FORMAT,
...
export default {
  ...
  apollo: {
    devopsAdoptionEnabledNamespaces: {
      ...
      error(error) {
        this.handleError(DA_ENABLED_NAMESPACE_QUERY_ERROR, error);
      },
    },
  },
  computed: {
  ...
  methods: {
      ...
      this.$apollo
        .mutate({
          ...
          update: (store, { data }) => {
            ...
            if (errors.length) {
              this.handleError(DA_ENABLE_NAMESPACE_MUTATION_ERROR, errors);
            } else {
        ...
        .catch((error) => {
          this.handleError(DA_ENABLE_NAMESPACE_MUTATION_ERROR, error);
        })
        .finally(() => {
    ...
    fetchGroups(searchTerm = '') {
      ...
      this.$apollo
        ...
        .catch((error) => this.handleError(DA_GROUPS_QUERY_ERROR, error));
    },
