...
import { transformStagesForPathNavigation, formatMedianValuesWithOverview } from '../utils';
...
export const pathNavigationData = ({ stages, medians, selectedStage }) => {
  const formattedMedians = formatMedianValuesWithOverview(medians);
  console.log('formattedMedians', formattedMedians);
  return transformStagesForPathNavigation({
    stages: [OVERVIEW_STAGE_CONFIG, ...filterStagesByHiddenStatus(stages, false)],
    medians: formatMedianValuesWithOverview(medians),
    selectedStage,
  });
};
