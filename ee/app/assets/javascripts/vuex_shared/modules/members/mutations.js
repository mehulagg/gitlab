...
export default {
  ...
  [types.RECEIVE_LDAP_OVERRIDE_ERROR](state, override) {
    if (override) {
      state.errorMessage = s__('Members|An error occurred while trying to enable LDAP override.');
    } else {
      state.errorMessage = s__(
        'Members|An error occurred while trying to revert to LDAP group sync settings.',
      );
    }
