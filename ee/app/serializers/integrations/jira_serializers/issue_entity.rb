  ...
  module JiraSerializers
    class IssueEntity < Grape::Entity
      ...
      expose :web_url do |jira_issue|
        project.jira_service.build_url("browse/#{jira_issue.key}")
      end
      ...
      def jira_web_url(user)
        ...
        if user['accountId'].present?
          project.jira_service.build_url("people/#{user['accountId']}")
        else
          project.jira_service.build_url('secure/ViewProfile.jspa', name: user['name'])
        end
      end
