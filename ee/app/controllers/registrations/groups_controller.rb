  ...
  class GroupsController < ApplicationController
    ...
    def apply_trial_flow
      if apply_trial
        ...
        experiment(:registration_group_invite, actor: @group) do |e|
          e.use { redirect_to new_users_sign_up_project_path(namespace_id: @group.id, trial: trial?, trial_onboarding_flow: true) } # control
          e.try(:invite_page) { redirect_to new_users_sign_up_group_invite_path(group_id: @group.id, trial: trial?, trial_onboarding_flow: true) } # with separate page
    ...
    def standard_onboarding_flow
      ...
      if experiment_enabled?(:trial_during_signup)
        trial_during_signup_flow if trial?
      else
        experiment(:registration_group_invite, actor: @group) do |e|
          e.use { invite_on_create } # control
          e.try(:invite_page) { redirect_to new_users_sign_up_group_invite_path(group_id: @group.id, trial: trial?) } # with separate page
          e.track(:created, property: @group.id)
        end
      end
    end
