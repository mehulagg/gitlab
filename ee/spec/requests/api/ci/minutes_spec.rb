...
RSpec.describe API::Ci::Minutes do
  ...
  describe 'PATCH /namespaces/:id/minutes/move/:target_id' do
    ...
    let_it_be(:unrelated_target) { create(:namespace) }
    let_it_be(:child_target) { create(:group, parent: namespace) }
    let_it_be(:user) { create(:user) }
    ...
    context 'when authorized' do
      ...
      context 'when the target namespace is not the root ancestor' do
        let(:target_id) { child_target.id }

        it 'returns an error' do
          move_packs

          expect(response).to have_gitlab_http_status(:bad_request)
          expect(response.body).to include('Target namespace must be a root namespace')
        end
      end
      ...
      context 'when the target namespace is not owned by the same user' do
        ...
        it 'returns an error' do
          ...
          expect(response).to have_gitlab_http_status(:bad_request)
          expect(response.body).to include('Target namespace must belong to the same owner')
        end
      end
      ...
      context 'when the transfer is successful' do
        ...
        it 'returns a success response' do
          expect { move_packs }.to change(target.ci_minutes_additional_packs, :count)
