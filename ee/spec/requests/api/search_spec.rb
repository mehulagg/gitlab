    ...
    context 'for commits and blobs', :sidekiq_inline do
      ...
      context 'for commits scope' do
        ...
        it 'avoids N+1 queries' do
          ...
          3.times do |i|
            commit_sha = project.repository.create_file(user, "#{i}", "folder #{i}", message: "committing folder #{i}", branch_name: project.default_branch)
      ...
      context 'for blobs scope' do
        ...
        it 'avoids N+1 queries' do
          ...
          project_2.repository.index_commits_and_blobs
          3.times do |i|
            commit_sha = project.repository.create_file(user, "#{i}", "Issue team #{i}", message: "#{i}", branch_name: project.default_branch)
            project.repository.commit(commit_sha)
          end
