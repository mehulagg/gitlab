...
RSpec.describe 'Namespace.projects' do
  ...
  describe 'sorting and pagination' do
    ...
    let(:project_args) { { include_subgroups: true, search: 'test' } }
    ...
    def pagination_query(params, page_info)
      arguments = params.merge(project_args)
    ...
    context 'when sorting by STORAGE' do
      ...
      it_behaves_like 'sorted pagable query' do
        ...
        def paging_query(params)
          graphql_query_for(:namespace, ns_args, query_graphql_field(:projects, params.merge(project_args), <<~GQL))
            #{page_info}
            nodes { name }
          GQL
        end
