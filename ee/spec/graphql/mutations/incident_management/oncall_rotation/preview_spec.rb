...
RSpec.describe Mutations::IncidentManagement::OncallRotation::Preview do
  ...
  describe '#resolve' do
    ...
    context 'user has access to project' do
      ...
      context 'when OncallRotation::CreateService responds with success' do
        it 'returns the on-call rotation with no errors' do
          expect(resolve).to match(
            errors: be_empty,
            oncall_rotation: having_attributes(
              class: IncidentManagement::OncallRotation,
              persisted?: false,
              schedule: schedule
            )
          )
      ...
      context 'when OncallRotations::PreviewService responds with an error' do
        before do
          allow_next_instance_of(::IncidentManagement::OncallRotations::PreviewService) do |service|
