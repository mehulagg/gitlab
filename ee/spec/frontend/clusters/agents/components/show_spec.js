...
describe('ClusterAgentShow', () => {
  ...
  const findCreatedText = () => wrapper.find('[data-testid="cluster-agent-create-info"]');
  const findTokenCount = () => wrapper.find('[data-testid="cluster-agent-token-count"]');
  ...
  it('displays agent create information', () => {
    expect(findCreatedText().text()).toMatchInterpolatedText('Created by user-1 2 days ago');
  });
  ...
  describe('when create user is unknown', () => {
    ...
    it('displays agent create information with unknown user', () => {
      expect(findCreatedText().text()).toMatchInterpolatedText(
        'Created by Unknown User 2 days ago',
      );
    });
  });

  it('displays token count', () => {
    expect(findTokenCount().text()).toMatchInterpolatedText(
  ...
  describe('when token count is missing', () => {
    ...
    it('displays token header with no count', () => {
      expect(findTokenCount().text()).toMatchInterpolatedText(`${ClusterAgentShow.i18n.tokens}`);
    });
  });
