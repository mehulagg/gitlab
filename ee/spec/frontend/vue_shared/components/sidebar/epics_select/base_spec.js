  ...
  describe('Base', () => {
    ...
    describe('methods', () => {
      ...
      describe('handleItemSelect', () => {
        ...
        it('should emit component event `onEpicSelect` with both `epicIssueId` & `issueId` props are not defined', async () => {
    ...
    describe('template', () => {
      ...
      it('should render DropdownHeader component when props `canEdit` & `showDropdown` are true', async () => {
        showDropdown();

        await wrapper.vm.$nextTick();
        expect(wrapper.find('[data-testid="epic-dropdown-header"]').exists()).toBe(true);
      });

      it('should not render DropdownHeader component when variant is "standalone"', async () => {
        showDropdown(wrapperStandalone);
        await wrapperStandalone.vm.$nextTick();
        expect(wrapperStandalone.find('[data-testid="epic-dropdown-header"]').exists()).toBe(false);
      });

      it('should render DropdownSearchInput component when props `canEdit` & `showDropdown` are true', async () => {
        showDropdown();
