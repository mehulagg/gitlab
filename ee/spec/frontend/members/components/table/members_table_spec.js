    ...
    describe('"Actions" field', () => {
      ...
      describe('when one of the members has `canOverride` permissions', () => {
        it('renders the "Actions" field', () => {
          ...
          const cssClasses = ['gl-display-none!', 'gl-display-lg-table-cell!'];
          expect(findTableCellByMemberId('Actions', memberNoPermissions.id).classes()).toEqual(
            expect.arrayContaining(cssClasses),
          );
          expect(findTableCellByMemberId('Actions', memberCanOverride.id).classes()).not.toEqual(
            expect.arrayContaining(cssClasses),
          );
        });
      });
