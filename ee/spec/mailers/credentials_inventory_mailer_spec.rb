...
RSpec.describe CredentialsInventoryMailer do
  ...
  describe '#personal_access_token_revoked' do
    let_it_be(:token) { create(:personal_access_token, scopes: %w(api sudo), last_used_at: 3.weeks.ago) }

    subject(:email) { described_class.personal_access_token_revoked(token: token, revoked_by: administrator) }
  ...
  describe '#ssh_key_deleted' do
    let_it_be(:ssh_key) { create(:personal_key, last_used_at: 3.weeks.ago) }

    subject(:email) { described_class.ssh_key_deleted(key: ssh_key, deleted_by: administrator) }
