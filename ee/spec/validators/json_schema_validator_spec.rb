...
RSpec.describe JsonSchemaValidator do
  describe '#validates_each' do
    ...
    let(:json_validator) { double(valid?: true) }

    subject(:validate_subject) { validator.validate_each(mock_subject, :foo, test_value) }

    before do
      allow(JSONSchemer).to receive(:schema).with(Pathname.new(schema_path)).and_return(json_validator)
    end
    ...
    context 'when the schema file exists on CE' do
      ...
      it 'calls the validator with CE schema' do
        ...
        expect(json_validator).to have_received(:valid?).with(test_value)
      end
    end
