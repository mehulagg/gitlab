...
RSpec.describe ApprovalRules::ExternalApprovalRule, type: :model do
  ...
  describe 'approved?' do
    ...
    subject { rule.approved?(merge_request) }

    context 'when a rule has a positive status check response' do
      let_it_be(:status_check_response) { create(:status_check_response, merge_request: merge_request, external_approval_rule: rule) }
