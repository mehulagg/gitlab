  ...
  describe '#update_metrics!' do
    ...
    context 'metric git_fetch_event_count_weekly' do
      it 'gets metrics from prometheus' do
        expect(prometheus_client).to receive(:query).with(Geo::SecondaryUsageData::GIT_FETCH_EVENT_COUNT_WEEKLY_QUERY).and_return([{ "value" => [1614029769.82, "48"] }])
        ...
        expect(described_class.last.git_fetch_event_count_weekly).to eq(48)
      end
