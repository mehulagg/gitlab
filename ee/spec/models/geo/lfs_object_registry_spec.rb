...
RSpec.describe Geo::LfsObjectRegistry, :geo do
  ...
  it_behaves_like 'a BulkInsertSafe model', Geo::LfsObjectRegistry do
    let(:valid_items_for_bulk_insertion) { build_list(:geo_lfs_object_registry_nonssf, 10) }
  ...
  describe '.find_registry_differences' do
    ...
    context 'untracked IDs' do
      before do
        create(:geo_lfs_object_registry_nonssf, lfs_object_id: lfs_object_1.id)
        create(:geo_lfs_object_registry_nonssf, :failed, lfs_object_id: lfs_object_3.id)
        create(:geo_lfs_object_registry_nonssf, lfs_object_id: lfs_object_4.id)
    ...
    context 'unused tracked IDs' do
      context 'with an orphaned registry' do
        let!(:orphaned) { create(:geo_lfs_object_registry_nonssf, lfs_object_id: non_existing_record_id) }
      ...
      context 'with selective sync by namespace' do
        ...
        context 'with a tracked LFS object' do
          let!(:registry_entry) { create(:geo_lfs_object_registry_nonssf, lfs_object_id: lfs_object_1.id) }
        ...
        context 'with a tracked LFS object' do
          context 'in object storage' do
            it 'includes tracked LFS object IDs that are in object storage' do
              create(:geo_lfs_object_registry_nonssf, lfs_object_id: lfs_object_remote_1.id)
