...
RSpec.describe GroupHook do
  ...
  describe '#rate_limit' do
    ...
    let_it_be(:group_hook) { create(:group_hook, group: group) }
    let_it_be(:group_hook_ultimate) { create(:group_hook, group: group_ultimate) }

    it 'returns the default limit for a group without a plan' do
      expect(group_hook.rate_limit).to be(100)
    end

    it 'returns the configured limit for a group with the Ultimate plan' do
      expect(group_hook_ultimate.rate_limit).to be(500)
    end
  end
end
