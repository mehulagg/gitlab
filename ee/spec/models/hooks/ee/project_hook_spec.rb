...
RSpec.describe EE::ProjectHook do
  describe '#rate_limit' do
    ...
    let_it_be(:project_hook_ultimate) { create(:project_hook, project: project_ultimate) }

    it 'returns the default limit for a project without a plan' do
      expect(project_hook.rate_limit).to be(100)
    end

    it 'returns the configured limit for a project with the Ultimate plan' do
      expect(project_hook_ultimate.rate_limit).to be(500)
    end
  end
end
