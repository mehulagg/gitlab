  ...
  describe '.secondary_replicables' do
    context 'without selective sync or object storage' do
      ...
      it 'excludes diffs stored in the database' do
        ...
        expect(described_class.secondary_replicables(1..described_class.last.id)).to be_empty
      end
    ...
    with_them do
      ...
      it 'returns the proper number of merge request diff states' do
        expect(described_class.secondary_replicables(1..described_class.last.id)).to have_attributes(count: synced_states)
      end
    end
  end
end
