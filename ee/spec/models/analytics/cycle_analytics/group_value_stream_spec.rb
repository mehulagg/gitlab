...
RSpec.describe Analytics::CycleAnalytics::GroupValueStream, type: :model do
  ...
  describe '#custom' do
    let(:default_name) { Analytics::CycleAnalytics::Stages::BaseService::DEFAULT_VALUE_STREAM_NAME }

    context 'when value stream is not persisted' do
      subject(:value_stream) { build(:cycle_analytics_group_value_stream, name: default_name) }

      it { is_expected.not_to be_custom }
    end

    context 'when value stream is persisted' do
      subject(:value_stream) { create(:cycle_analytics_group_value_stream, name: 'test') }

      it { is_expected.to be_custom }

      context 'when when the name of the value stream is default' do
        subject(:value_stream) { create(:cycle_analytics_group_value_stream, name: default_name) }
