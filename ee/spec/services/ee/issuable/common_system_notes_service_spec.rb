...
RSpec.describe Issuable::CommonSystemNotesService do
  ...
  context 'on issuable update' do
    subject { described_class.new(container: project, current_user: user).execute(issuable, old_labels: []) }
    ...
    context 'when issuable is an epic' do
      ...
      subject { described_class.new(container: nil, current_user: user).execute(issuable, old_labels: []) }
  ...
  context 'on issuable create' do
    ...
    subject { described_class.new(container: project, current_user: user).execute(issuable, old_labels: [], is_update: false) }
