    ...
    context 'when entity is an epic' do
      ...
      context 'when issue has weight events' do
        it 'ignores copying weight events' do
          resource_weight_events = create_list(:resource_weight_event, 2, issue: original_issue)
          expect(subject).not_to receive(:copy_events).with(ResourceWeightEvent.table_name, resource_weight_events)
