...
RSpec.describe MemberPermissionExport::ExportService do
  let(:service) { described_class.new(current_user, upload) }

  let_it_be(:admin) { create(:admin) }
  let(:upload) { create(:user_permission_export_upload, user: admin) }
  let_it_be(:user) { create(:user, username: 'Jessica', email: 'jessica@test.com') }

  context 'access' do
    shared_examples 'allowed to export user permissions' do
      it { expect(service.export).to be_success }
    end

    shared_examples 'not allowed to export user permissions' do
      it { expect(service.export).not_to be_success }
    end
  ...
  context 'data verification', :enable_admin_mode do
    subject(:csv) { CSV.parse(upload.file.read, headers: true) }
