  ...
  describe '#execute' do
    ...
    context 'when container is project' do
      ...
      context 'when data range is too wide' do
        let(:extra_params) { { after: 1.year.ago.to_date } }
      ...
      context 'when after date is later than before date' do
        let(:extra_params) { { before: 1.year.ago.to_date } }

        it_behaves_like 'request failure' do
          let(:message) { 'The after date must be ealier than the before date.' }
          let(:http_status) { :bad_request }
        end
      end
    ...
    context 'when container is group' do
      ...
      it_behaves_like 'request failure' do
        let(:message) { 'Unsupported container level.' }
        let(:http_status) { :bad_request }
      end
    end
  end
