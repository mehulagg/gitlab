  ...
  describe '#execute' do
    ...
    context 'by scanner' do
      context 'when unscoped' do
        ...
        it 'returns all vulnerabilities with all scanners available' do
          expect(subject.findings.map(&:scanner).map(&:external_id).uniq).to match_array %w[bundler_audit find_sec_bugs gemnasium container-scanning zaproxy]
        end
      end
    ...
    context 'by all filters' do
      context 'with found entity' do
        let(:params) { { report_type: %w[sast dast container_scanning dependency_scanning], scanner: %w[bundler_audit find_sec_bugs gemnasium container-scanning zaproxy], scope: 'all' } }
