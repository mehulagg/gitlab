...
RSpec.describe Analytics::DevopsAdoption::SegmentsFinder do
  ...
  subject { described_class.new(admin_user, params: params).execute }
  let(:params) { {} }
  ...
  describe '#execute' do
    ...
    context 'for instance level' do
      it 'returns segments ordered by name' do
        expect(subject).to eq([segment_2, segment_1, direct_subgroup_segment, indirect_subgroup_segment])
      end
    ...
    context 'for group level' do
      ...
      it 'returns segments scoped to given namespace ordered by name' do
        expect(subject).to eq([segment_1, direct_subgroup_segment, indirect_subgroup_segment])
      end
      ...
      context 'with direct_descendants_only' do
        ...
        it 'returns direct descendants only' do
          expect(subject).to eq([segment_1, direct_subgroup_segment])
        end
      end
    end
  end
