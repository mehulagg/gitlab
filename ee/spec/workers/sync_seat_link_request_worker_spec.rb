...
RSpec.describe SyncSeatLinkRequestWorker, type: :worker do
  describe '#perform' do
    ...
    context 'when response contains reconciliation dates' do
      let(:body) { { success: true, next_reconciliation_date: Date.current.to_s, display_alert_from: (Date.current - 7.days).to_s }.to_json }
      ...
      it 'saves the reconciliation dates' do
        ...
        expect(upcoming_reconciliation.next_reconciliation_date).to eq(Date.current)
        expect(upcoming_reconciliation.display_alert_from).to eq(Date.current - 7.days)
      end

      context 'when an upcoming_reconciliation already exists' do
        it 'updates the upcoming_reconciliation' do
          create(:upcoming_reconciliation, :self_managed, next_reconciliation_date: Date.current + 2.days, display_alert_from: Date.current + 1.day)
