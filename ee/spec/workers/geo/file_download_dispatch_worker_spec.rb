    ...
    context 'with files missing on the primary' do
      ...
      context 'with lfs_object_registry entries' do
        before do
          ...
          Geo::LfsObjectRegistry.create!(lfs_object_id: lfs_object_file_missing_on_primary.id, bytes: 1234, success: true, missing_on_primary: true, created_at: Time.now.utc)
        end
