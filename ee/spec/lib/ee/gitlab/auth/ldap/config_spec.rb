  ...
  describe '.providers' do
    ...
    context 'when multiple LDAP servers are licensed' do
      ...
      it 'returns multiple configured providers' do
        expect(described_class.providers).to match_array(%w(ldapmain ldapsecondary))
      end
    end

    context 'when multiple LDAP servers are not licensed' do
      before do
        stub_licensed_features(multiple_ldap_servers: false)
      end

      it 'returns multiple configured providers' do
        expect(described_class.providers).to match_array(%w(ldapmain))
      end
    end
  end
