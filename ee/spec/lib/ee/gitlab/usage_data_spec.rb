...
RSpec.describe Gitlab::UsageData do
  ...
  describe 'usage_data_by_stage_enablement' do
    ...
    it 'excludes data outside of the date range' do
      create_list(:geo_node, 10).each do |node|
        for_defined_days_back do
          create(:oauth_access_grant, application: node.oauth_application)
        end
      end

      expect(described_class.usage_activity_by_stage_enablement(described_class.last_28_days_time_period)).to eq({:geo_secondary_web_oauth_users=>10})
    end
  end
