...
RSpec.describe EE::Gitlab::Checks::PushRules::TagCheck do
  ...
  describe '#validate_change!' do
    ...
    it 'returns an error if the rule denies tag deletion' do
      expect { subject.validate_change!(oldrev, newrev, ref) }.to raise_error(Gitlab::GitAccess::ForbiddenError, 'You cannot delete a tag')
    end
    ...
    context 'when tag is deleted in web UI' do
      ...
      it 'ignores the push rule' do
        expect(subject.validate_change!(oldrev, newrev, ref)).to be_truthy
      end
    end
  end
end
