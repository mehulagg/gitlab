...
RSpec.describe OmniAuth::Strategies::GroupSaml, type: :strategy do
  ...
  describe 'POST /groups/:group_path/-/saml/callback' do
    context 'with valid SAMLResponse' do
      before do
        allow_next_instance_of(OneLogin::RubySaml::Response) do |response|
          allow(response).to receive(:validate_signature) { true }
          allow(response).to receive(:validate_session_expiration) { true }
          allow(response).to receive(:validate_subject_confirmation) { true }
          allow(response).to receive(:validate_conditions) { true }
        end
      end
      ...
      context 'user is testing SAML response' do
        ...
        it 'stores the saml response for retrieval after redirect' do
          expect_next_instance_of(::Gitlab::Auth::GroupSaml::ResponseStore) do |response|
            expect(response).to receive(:set_raw).with(saml_response)
          end
