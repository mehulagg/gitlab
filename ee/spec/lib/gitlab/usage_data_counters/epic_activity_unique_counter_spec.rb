...
RSpec.describe Gitlab::UsageDataCounters::EpicActivityUniqueCounter, :clean_gitlab_redis_shared_state do
  ...
  context 'for epic date modification events' do
    ...
    context 'end date' do
      context 'setting as fixed event' do
        def track_action(params)
          described_class.track_epic_end_date_set_as_fixed_action(**params)
        end

        it_behaves_like 'a daily tracked issuable event' do
          let(:action) { described_class::EPIC_END_DATE_SET_AS_FIXED }
        end
      ...
      context 'setting as inherited event' do
        def track_action(params)
          described_class.track_epic_end_date_set_as_inherited_action(**params)
        end

        it_behaves_like 'a daily tracked issuable event' do
          let(:action) { described_class::EPIC_END_DATE_SET_AS_INHERITED }
        end
