...
RSpec.describe BillingPlansHelper do
  ...
  describe '#should_show_eoa_banner' do
    let_it_be(:user) { create(:user) }
    stub_feature_flags(show_billing_eoa_banner: true)

    before do
      allow(helper).to receive(:eoa_date).and_return(Date.current + 5.days)
    end
    ...
    shared_examples 'current time' do
      ...
      it 'displays the banner' do
        expect(helper.should_show_eoa_banner(namespace)).to eq(true)
      end
    end
    ...
    shared_examples 'past eoa date' do
      ...
      it 'does not display the banner' do
        travel_to(eoa_date + 1.day) do
          expect(helper.should_show_eoa_banner(namespace)).to eq(false)
        end
      end
    end
