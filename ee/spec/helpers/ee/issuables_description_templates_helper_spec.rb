...
RSpec.describe IssuablesDescriptionTemplatesHelper do
  ...
  RSpec.shared_examples 'project issuable templates' do
    ...
    def templates(issuable_type, inherited_from)
      [
        { id: "#{issuable_type}-bar", name: "#{issuable_type}-bar", namespace_path: inherited_from.namespace.full_path, project_path: inherited_from.path },
        { id: "#{issuable_type}-foo", name: "#{issuable_type}-foo", namespace_path: inherited_from.namespace.full_path, project_path: inherited_from.path }
      ]
    end
  end

  describe '#project_issuable_templates' do
    let(:inherited_from) { nil }
    ...
    context 'when project has parent group' do
      ...
      context 'when project parent group has a file template project' do
        ...
        let!(:project) { create(:project, :custom_repo, group: group, files: issue_template_files) }
        let(:inherited_from) { file_template_project }
