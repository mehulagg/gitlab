...
module Gitlab
  module Geo
    ...
    module VerificationState
      ...
      VERIFICATION_TIMEOUT = 8.hours.ago

      included do
        sha_attribute :verification_checksum

        # rubocop:disable CodeReuse/ActiveRecord
        scope :verification_pending, -> { with_state(:verification_pending) }
        scope :verification_started, -> { with_state(:verification_started) }
        scope :verification_succeeded, -> { with_state(:verification_succeeded) }
        scope :verification_failed, -> { with_state(:verification_failed) }
        ...
        scope :verification_expired, -> { where("verification_started_at < ?", VERIFICATION_TIMEOUT.ago) }
