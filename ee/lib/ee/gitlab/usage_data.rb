    ...
    module UsageData
      ...
      class_methods do
        ...
        def usage_activity_by_stage_secure(time_period)
          ...
          results[:"#{prefix}#{SECURE_PRODUCT_TYPES[:coverage_fuzzing][:name]}".to_sym] = distinct_count(::Ci::Build.where("options ILIKE ?", "%gitlab-cov-fuzz%").or(name: secure_type).where(time_period),
