...
export default {
  ...
  computed: {
    isProtectedBranch() {
      return parseBoolean(this.modalData.isProtectedBranch);
    },
    actionText() {
      return this.isProtectedBranch ? __('Reject') : __('Remove Branch');
    },
    actionPrimary() {
      return {
  ...
  methods: {
    handleClick(event) {
      const removeButton = event.target.closest('.js-delete-branch-button');
      if (removeButton) {
