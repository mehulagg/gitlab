...
export default {
  ...
  data() {
    return {
      selectedTabIndex: 0,
      initialTabIndexCalculated: false,
    };
  },
  computed: {
    ...mapState({
  ...
  mounted() {
    // To prevent any flashing of content and improve performance wait until
    // URL params have been parsed before rendering the tabs
    if (this.activeTabIndexCalculatedFromUrlParams === -1) {
      this.initialTabIndexCalculated = true;
      return;
    }
...
<template>
  <gl-tabs v-if="initialTabIndexCalculated" v-model="selectedTabIndex">
