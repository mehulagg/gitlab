    ...
    <tr v-for="section in file.parallelLines" class="line_holder parallel">
      <template v-for="line in section">
        <template v-if="line.isHeader">
          <td
            :key="`${line.id + line.richText}-placeholder`"
            class="diff-line-num header"
            :class="lineCssClass(line)"
          ></td>
          <td
            :key="`${line.id + line.richText}-text`"
            class="line_content header"
            :class="lineCssClass(line)"
          >
        ...
        <template v-else>
          <td
            :key="`${line.lineNumber + line.richText}-lineNumber`"
          ...
          <td
            :key="`${line.lineNumber + line.richText}-text`"
