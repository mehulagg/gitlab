...
export const setFileResolveMode = ({ commit, state }, { file, mode }) => {
  const index = state.conflictsData.files.findIndex((f) => f.blobPath === file.blobPath);
...
export const handleSelected = ({ commit, state }, { file, line: { id, section } }) => {
  const index = state.conflictsData.files.findIndex((f) => f.blobPath === file.blobPath);
  const updated = { ...state.conflictsData.files[index] };
