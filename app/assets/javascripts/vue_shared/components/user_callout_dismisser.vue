<script>
import dismissUserCalloutQuery from '~/graphql_shared/mutations/dismiss_user_callout.mutation.graphql';
 ...
 *      if the query has loaded without error and the user callout has not been
 *      dismissed yet; `false` otherwise.
 */
export default {
  ...
  computed: {
    ...
    isAnonUser() {
      return !this.isLoadingQuery && !this.currentUser;
    },
    isDismissedRemote() {
    ...
    async dismiss() {
      ...
      try {
        const { data } = await this.$apollo.mutate({
          mutation: dismissUserCalloutQuery,
          variables: {
            input: {
        ...
        this.onDismissalError([__('Failed to dismiss user callout')]);
      } finally {
