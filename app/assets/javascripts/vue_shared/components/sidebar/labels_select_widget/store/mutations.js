import { isScopedLabel, scopedLabelBaseValue } from '~/lib/utils/common_utils';
  ...
  [types.UPDATE_SELECTED_LABELS](state, { labels }) {
    ...
    if (candidateLabel) {
      ...
      if(isScopedLabel(candidateLabel)) {
        const scopedBase = scopedLabelBaseValue(candidateLabel);
        const currentActiveScopedLabel = state.labels.find(({ title }) => title.indexOf(scopedBase) === 0 && ( title !== candidateLabel.title ));
