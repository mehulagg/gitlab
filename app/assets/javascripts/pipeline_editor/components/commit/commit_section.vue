  ...
  methods: {
    ...
    async onCommitSubmit(event) {
      ...
      try {
        ...
        if (errors?.length) {
          this.$emit('showAlert', { type: COMMIT_FAILURE, reasons: errors });
          return;
        }
        ...
        if (openMergeRequest) {
          ...
          this.$emit('showAlert', { type: COMMIT_SUCCESS });
        }
      } catch (error) {
        this.$emit('showAlert', { type: COMMIT_FAILURE, reasons: [error?.message] });
      } finally {
        this.isSaving = false;
      }
    },
