...
export default {
  ...
  data() {
    return {
      ciConfigData: {},
      ciFileContent: '',
      // ciFilContentModel is either the graphQL data fetched on load or
      // the new data written by the user in the editor
      ciFileContentModel: '',
  ...
  apollo: {
    ciFileContent: {
      ...
      result({ data }) {
        this.ciFileContentModel = data?.blobContent?.rawData ?? '';
      },
      error(error) {
    ...
    ciConfigData: {
      ...
      skip: ({ ciFileContentModel }) => {
        return !ciFileContentModel;
      },
      variables() {
        return {
          projectPath: this.projectFullPath,
          content: this.ciFileContentModel,
        };
      },
      update(data) {
        const { ciConfig } = data || {};
  ...
  computed: {
    isBlobContentLoading() {
      return this.$apollo.queries.ciFileContent.loading;
    },
    isBlobContentError() {
  ...
  methods: {
    ...
    showAlert({ type, reasons = [] }) {
      if (this.$options.successTexts[type]) {
        this.reportSuccess(type);
      } else if (this.$options.errorTexts[type]) {
        this.reportFailure(type, reasons);
      }
    },
    updateCiConfig(ciFileContent) {
      this.ciFileContentModel = ciFileContent;
    },
  },
};
</script>
  ...
  <div class="gl-mt-4">
    ...
    <div v-else-if="!isBlobContentError" class="gl-mt-4">
      <pipeline-editor-home
        ...
        :ci-file-content="ciFileContentModel"
        @showAlert="showAlert"
