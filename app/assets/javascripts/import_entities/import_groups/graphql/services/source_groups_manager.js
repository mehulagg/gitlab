...
export class SourceGroupsManager {
  ...
  saveImportStateToStorage(importId, group) {
  ...
  startImport({ group, importId }) {
    this.saveImportStateToStorage(importId, group);
  ...
  setImportStatusByImportId(importId, status) {
    ...
    if (group?.id) {
      this.update(group, (sourceGroup) => {
        // eslint-disable-next-line no-param-reassign
        sourceGroup.status = status;
        if (!isEqual(group.import_target, importState.importTarget)) {
          // eslint-disable-next-line no-param-reassign
          sourceGroup.import_target = importState.importTarget;
        }
      });
    }
