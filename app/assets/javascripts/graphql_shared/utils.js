 ...
 * and converts the `id` property to a number.
 *
 * @param {Array} nodesOrIds An array of nodes or GraphQL IDs
 * @returns {Array}
 */
export const convertFromGraphQLIds = (nodesOrIds) =>
  nodesOrIds.map((nodeOrId) => {
    // Check if it is a node with `id` attribute or an ID
    if (isObject(nodeOrId)) {
      const node = nodeOrId;
      return node.id ? { ...node, id: getIdFromGraphQLId(node.id) } : node;
    }

    return getIdFromGraphQLId(nodeOrId);
  });
