...
export default {
  components: { LearnGitlabInfoCard, GlProgressBar },
  i18n: {
  ...
  methods: {
    ...
    progressValue() {
      return Object.entries(this.actions).filter(([_, a]) => a.completed).length;
    },
    progressPercentage() {
...
<template>
  <div>
    <div class="row">
      <div class="gl-mb-5 col-md-8 col-lg-7">
    ...
    <div class="gl-mb-3">
      <p class="gl-text-gray-500 gl-mb-2" data-testid="completion-percentage">
        {{ this.progressPercentage() }}% completed
      </p>
      <gl-progress-bar :value="progressValue()" :max="$options.maxValue" />
    </div>

    <h2 class="gl-font-lg">{{ $options.i18n.workspace.title }}</h2>
    <p class="gl-text-gray-700">{{ $options.i18n.workspace.description }}</p>
    ...
    <h2 class="gl-font-lg">{{ $options.i18n.plan.title }}</h2>
    <p class="gl-text-gray-700">{{ $options.i18n.plan.description }}</p>
    ...
    <h2 class="gl-font-lg">{{ $options.i18n.deploy.title }}</h2>
    <p class="gl-text-gray-700">{{ $options.i18n.deploy.description }}</p>
