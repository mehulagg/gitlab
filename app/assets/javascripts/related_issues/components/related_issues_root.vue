        ...
        RelatedIssuesService.remove(issueToRemove.relationPath)
          ...
          .catch((res) => {
            if (res && res.status !== 404) {
              createFlash({
                message: relatedIssuesRemoveErrorMap[this.issuableType],
              });
            }
          });
      } else {
        createFlash({
          message: pathIndeterminateErrorMap[this.issuableType],
        });
      }
    },
    ...
    saveIssueOrder({ issueId, beforeId, afterId, oldIndex, newIndex }) {
      ...
      if (issueToReorder) {
        RelatedIssuesService.saveOrder({
          ...
          .catch(() => {
            createFlash({
              message: __('An error occurred while reordering issues.'),
            });
          });
      }
    },
