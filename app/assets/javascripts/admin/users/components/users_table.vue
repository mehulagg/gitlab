  ...
  apollo: {
    groupCounts: {
      ...
      update(data) {
        ...
        return Object.fromEntries(parsedIds.map(({ id, groupCount }) => [id, groupCount || 0]));
      },
      error(error) {
        createFlash({ message: this.$options.i18n.groupCountFetchError });
    ...
    >
      ...
      <template #cell(groupCount)="{ item: { id } }">
        <div :data-testid="`user-group-count-${id}`">
          <gl-skeleton-loader v-if="!groupCounts" :width="40" :lines="1" />
