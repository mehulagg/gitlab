...
export default {
  ...
  watch: {
    ...
    diffFiles() {
      this.fileReviews = this.diffFiles.map(file => getReviewForFile(this.allReviews, file));
    },
    diffViewType() {
      this.adjustView();
    },
  ...
  methods: {
    ...
    reviewFile({ file } = {}) {
      const fileIndex = this.diffFiles.findIndex(diffFile => diffFile.id === file.id);
      ...
      this.fileReviews[fileIndex] = getReviewForFile(reviews, file);
    },
