  ...
  computed: {
    ...
    commentLineOptions() {
      const combineSides = (acc, { left, right }) => {
        ...
        if (left && left.type !== 'match' && this.linePosition === 'left') acc.push(left);
        // if the line_codes are identically, return to avoid duplicates
        if (left?.line_code === right?.line_code && this.linePosition === 'left') return acc;
        if (right && right.type !== 'match' && this.linePosition === 'right') acc.push(right);
        return acc;
      };
      const getDiffLines = () => {
