...
export const selectedContextCommitsDiff = (state) =>
...
export const diffCompareDropdownSourceVersions = (state, getters) => {
  ...
  const versions = state.mergeRequestDiffs.map((v, i) => {
    ...
    return {
      ...
      selected: v.version_index === getters.selectedSourceIndex && !getters.selectedContextCommitsDiff,
    };
  });
  ...
  if (contextCommitsDiff) {
    versions.push({
      ...
      selected: getters.selectedContextCommitsDiff,
