  ...
  return new Vue({
    ...
    created() {
      ...
      if (Cookies.get(DIFF_WHITESPACE_COOKIE_NAME)) {
        ...
        this.setShowWhitespace({ url: this.endpointUpdateUser, showWhitespace: hideWhitespace !== '1' });
        Cookies.remove(DIFF_WHITESPACE_COOKIE_NAME);
      } else {
        // This is only to set the the user preference in Vuex for use later
        this.setShowWhitespace({ showWhitespace: this.showWhitespaceDefault, updateDatabase: false });
      }
    },
    methods: {
