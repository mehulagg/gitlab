...
const USER_COLLAPSED_SIDEBAR_COOKIE = 'collapsed_gutter';
...
export default {
  ...
  data() {
    const userExpanded = !parseBoolean(Cookies.get(USER_COLLAPSED_SIDEBAR_COOKIE));
  ...
  methods: {
    ...
    toggleSidebar() {
      ...
      Cookies.set(USER_COLLAPSED_SIDEBAR_COOKIE, !this.userExpanded);
      this.updatePageContainerClass();
    },
  },
};
  ...
  >
    ...
    <div data-testid="sidebar-items" class="issuable-sidebar">
      <slot
        ...
        v-bind="{ sidebarExpanded: isExpanded, toggleSidebar: toggleSidebar }"
      ></slot>
    </div>
  </aside>
</template>
