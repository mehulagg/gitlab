    ...
    fetchKeys() {
      ...
      return this.service
        ...
        .catch(() => {
          this.isLoading = false;
          this.store.keys = {};
          return createFlash({ message: s__('DeployKeys|Error getting deploy keys') });
        });
    },
    enableKey(deployKey) {
      this.service
        ...
        .then(this.fetchKeys)
        .catch(() => createFlash({ message: s__('DeployKeys|Error enabling deploy key') }));
    },
    confirmRemoveKey(deployKey, callback) {
      ...
      this.removeKey = () => {
        this.service
          ...
          .then(this.fetchKeys)
          .then(hideModal)
          .catch(() => createFlash({ message: s__('DeployKeys|Error removing deploy key') }));
      };
