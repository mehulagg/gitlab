...
module GroupsHelper
  ...
  def prevent_sharing_groups_outside_hierarchy_help_text(group)
    ...
    s_("GroupSettings|This setting is only available on the top-level group and it applies to all subgroups. Groups that have already been shared with a group outside %{group} will still be shared, and this access will have to be revoked manually.").html_safe % { group: group_link }
  end
