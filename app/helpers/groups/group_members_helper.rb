...
module Groups::GroupMembersHelper
  ...
  def group_members_list_data_json(group, user:, invite:, access_request:)
    {
      user: group_members_list_data(group, user, { param_name: :page, params: { invited_members_page: nil, search_invited: nil } }),
      group: group_group_links_list_data(group),
      invite: group_members_list_data(group, invite.nil? ? [] : invite, { param_name: :invited_members_page, params: { page: nil } }),
      access_request: group_members_list_data(group, access_request.nil? ? [] : access_request),
      source_id: group.id,
