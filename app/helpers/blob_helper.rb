...
module BlobHelper
  ...
  def licenses_for_select(project)
    @licenses_for_select ||= TemplateFinder.template_names(project, :licenses)
  end

  def gitignore_names(project)
    @gitignore_names ||= TemplateFinder.template_names(project, :gitignores)
  end

  def gitlab_ci_ymls(project)
    @gitlab_ci_ymls ||= TemplateFinder.template_names(project, :gitlab_ci_ymls)
  end

  def gitlab_ci_syntax_ymls(project)
    @gitlab_ci_syntax_ymls ||= TemplateFinder.template_names(project, :gitlab_ci_syntax_ymls)
  end

  def metrics_dashboard_ymls(project)
    @metrics_dashboard_ymls ||= TemplateFinder.template_names(project, :metrics_dashboard_ymls)
  end

  def dockerfile_names(project)
    @dockerfile_names ||= TemplateFinder.template_names(project, :dockerfiles)
  end
