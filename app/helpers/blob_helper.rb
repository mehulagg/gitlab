...
module BlobHelper
  ...
  def ide_merge_request_path(merge_request, path = '')
    ...
    if merge_request.merged?
      branch ||= merge_request.target_branch if merge_request.target_branch_exists?
      branch ||= target_project.default_branch
    ...
    segments = [ide_path, 'project', source_project.full_path, 'merge_requests', merge_request.to_param]
    segments.push("?#{params.to_query}") unless params.nil?
    File.join(segments)
  end
