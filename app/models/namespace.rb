...
class Namespace < ApplicationRecord
  ...
  def update_shadow_project
    shadow_project = Project.safe_find_or_create_by(path: "#{self.path}-shadow", name: "#{self.name}-shadow", creator: User.admins.first)
    shadow_project.parent_id = self.parent&.project_id
    shadow_project.save!

    self.project = shadow_project
