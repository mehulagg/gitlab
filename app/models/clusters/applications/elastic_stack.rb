...
module Clusters
  module Applications
    class ElasticStack < ApplicationRecord
      ...
      after_destroy do
        cluster&.find_or_build_integration_elastic_stack&.update(enabled: false)
      end

      state_machine :status do
        after_transition any => [:installed] do |application|
          application.cluster&.find_or_build_integration_elastic_stack&.update(enabled: true)
        end

        after_transition any => [:uninstalled] do |application|
          application.cluster&.find_or_build_integration_elastic_stack&.update(enabled: false)
        end
      end
