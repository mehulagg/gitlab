...
module Ci
  class BuildDependencies
    ...
    CROSS_PIPELINE_LIMIT = ::Gitlab::Ci::Config::Entry::Needs::NEEDS_CROSS_PIPELINE_DEPENDENCIES_LIMIT
    ...
    def jobs_in_pipeline_hierarchy(deps_specifications)
      ...
      deps_specifications.each do |spec|
        all_pipeline_ids << spec[:pipeline].to_i
      ...
      model_class.latest.success
        ...
        .limit(CROSS_PIPELINE_LIMIT)
        .select do |dependency|
          # the query may not return exact matches pipeline-job because we filter them
          # separately.
          deps_specifications.find do |spec|
            spec[:pipeline].to_i == dependency.pipeline_id &&
    ...
    def expand_variables_and_validate(specifications)
      specifications.map do |spec|
        pipeline = ExpandVariables.expand(spec[:pipeline].to_s, processable_variables)
        # current pipeline is not allowed because local dependencies
        # should be used instead.
        next if pipeline.to_i == processable.pipeline_id
    ...
    def specified_cross_pipeline_dependencies
      return [] unless Feature.enabled?(:ci_cross_pipeline_artifacts_download, processable.project, default_enabled: false)
