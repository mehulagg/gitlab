...
module Users
  class ApproveService < BaseService
    ...
    def execute(user)
      ...
      return error(_('The user you are trying to approve is not pending approval'), :conflict) if user.active?
      return error(_('The user you are trying to approve is not pending approval'), :conflict) unless approval_required?(user)
