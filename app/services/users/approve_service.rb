...
module Users
  class ApproveService < BaseService
    ...
    def log_event(user)
      ::Gitlab::AppLogger.info "USER INSTANCE REQUEST APPROVED: user: #{user.username}, email: #{user.email}, approved_by: #{current_user.username}, ip_address=#{current_user.current_sign_in_ip}"
    end
  end
end
