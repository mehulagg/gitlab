  ...
  class RefreshAuthorizedProjectsService
    ...
    def log_refresh_details(remove, add)
      Gitlab::AppJsonLogger.info(event: 'authorized_projects_refresh',
                                 ...
                                 # most often there's only a few entries in remove and add but let's not flood the logs,
                                 # so limit it to the first 5 entries
                                 'authorized_projects_refresh.rows_deleted': remove.first(5),
                                 'authorized_projects_refresh.rows_added': add.first(5))
    end
