...
module Namespaces
  class InProductMarketingEmailsService
    ...
    def record_sent_emails
      Namespaces::InProductMarketingEmail.bulk_insert!(in_product_marketing_email_records)
    end

    def track_sent_email(user, group, track, series)
      current_batch_user_ids << user.id

      in_product_marketing_email_records << Namespaces::InProductMarketingEmail.new(
        user: user,
        namespace: group,
        track: track,
        series: series,
