...
module Members
  class InviteService < Members::BaseService
    ...
    def execute(source)
      return error(s_('No users specified.')) if params[:email].blank?
      ...
      emails.each do |email|
        ...
        if existing_user.present?
          ...
          if new_member.valid? && new_member.persisted?
            ...
            errors << new_member.errors.full_messages.to_sentence
          end
          next
        end
        ...
        if new_invite.valid? && new_invite.persisted?
          after_execute(member: new_invite)
        else
          current_error =
