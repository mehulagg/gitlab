...
class Ci::PipelineEntity < Grape::Entity
  ...
  expose :flags do
    ...
    expose :merged_request_pipeline?, as: :merge_request_pipeline
  end
  ...
  expose :source_sha, if: -> (pipeline, _) { pipeline.merged_request_pipeline? }
  expose :target_sha, if: -> (pipeline, _) { pipeline.merged_request_pipeline? }
