...
module Packages
  module Nuget
    class ServiceIndexPresenter
      ...
      PUSH_SERVICES = %i[publish].freeze
      PULL_SERVICES = %i[download search metadata].freeze
      ...
      def available_services
        ...
        when :group
          PULL_SERVICES
        when :project
          (PULL_SERVICES + PUSH_SERVICES).flatten
        end
      end
      ...
      def download_service_url
        ...
        case scope
        when :group
          api_v4_groups_packages_nuget_download_package_name_package_version_package_filename_path(
            params,
            true
          )
        when :project
