...
to the end of the GitHub authorization callback URL. Note that GitHub doesn't validate that the subdomain
part of the `redirect_uri` matches the authorization callback URL before redirecting the user during the
OAuth flow, this means that an XSS or an open redirect on any subdomain of your website could enable
