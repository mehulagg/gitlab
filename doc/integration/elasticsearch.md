...
| [`sudo gitlab-rake gitlab:elastic:clear_index_status`](https://gitlab.com/gitlab-org/gitlab/blob/master/ee/lib/tasks/gitlab/elastic.rake)               | Deletes all instances of IndexStatus for all projects. Note that this command will result in complete wipe of the index, and it should be used with caution. We recommend using [Trigger the reindex via the Advanced Search administration](#trigger-the-reindex-via-the-advanced-search-administration) as it is much more efficient than indexing from scratch.                                                                                              |
