...
Do NOT backup or restore GitLab through a PgBouncer connection. The
backup task must bypass PgBouncer and [connect directly to the
PostgreSQL primary database node](../administration/postgresql/pgbouncer.md#procedure-for-bypassing-pgbouncer),
or it will cause a GitLab outage.

When the GitLab backup or restore task is used with PgBouncer, the
following error message is shown when performing backups and restores:
...
path and explicitly includes the schema in every SQL
query](https://gitlab.com/gitlab-org/gitlab/-/issues/23211) to address
...
By default, the GitLab uses the database configuration stored in a
configuration file (`database.yml`). However, the database settings can
be overidden by the backup and restore task by setting environment
...
and port 5432 using the Omnibus package:
