...
- Using the [GitLab managed Terraform State](#gitlab-managed-terraform-state) as the Terraform state storage backend
- Creates [4 pipeline stages](https://gitlab.com/gitlab-org/gitlab/-/blob/master/lib/gitlab/ci/templates/Terraform.latest.gitlab-ci.yml): init, validate, build and deploy. These [stages are basically running Terraform commands](https://gitlab.com/gitlab-org/gitlab/-/blob/master/lib/gitlab/ci/templates/Terraform/Base.latest.gitlab-ci.yml): `init`, `validate`, `plan`, `plan-json` and `apply`. The latter only on master.
