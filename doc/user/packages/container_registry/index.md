...
When [pushing a Docker manifest list](https://docs.docker.com/engine/reference/commandline/manifest/#create-and-push-a-manifest-list) to the GitLab Container Registry, you may see the error `manifest blob unknown: blob unknown to registry` if the individual child manifests referenced in the manifest list were not pushed to the same repository.

For example, if you have two individual images, one for `amd64` and another for `arm64v8`, and you want to build a multi-arch image with these, the `amd64` and `arm64v8` images must be pushed to the same repository where you want to push the multi-arch image.

For this reason, it's advised to include the architecture in the tag name of individual images, such as `mygroup/myapp:1.0.0-amd64`, instead of using sub repositories, e.g. `mygroup/myapp/amd64:1.0.0`. You can then tag the manifest list with `mygroup/myapp:1.0.0`.
