...
WARNING:
GitLab Self-Managed installs support third-party container registries that comply with the [Docker Registry HTTP API V2](https://docs.docker.com/registry/spec/api/) specification. However, this specification does not include a tag delete operation. 

For this reason, when interacting with third-party container registries, GitLab will attempt to use a workaround (see the [related issue](https://gitlab.com/gitlab-org/gitlab/-/issues/15737) for more details) to delete tags. However, due to possible implementation variations, this workaround is not guaranteed to work with all third-party registries in the same predictable way. 

If you are using the GitLab Container Registry, this workaround is not required as we have implemented a special tag delete operation, in which case you can expect cleanup policies to be consistent, reproducible, and performant.
