...
1. In the `sast-ruleset.toml` file, do one of the following:
   ...
   - Disable predefined rules belonging to SAST analyzers. In this example, the rules belong to
     `eslint` and `sobelow` with the corresponding identifier `type` and `value` have been disabled:
