...
The artifact generated by the secure analyzer will contain both new and existing findings (it puts in everything that it finds).
...
It is recommended you run one set of scans before enabling your scans for developers to create your backlog, and not have the first developer get all the results in that first scan.
...
When you click “view full report” - you are taken to the `Security` tab in the pipeline.
...
The vulnerability report shows the results of the last completed pipeline on the default branch. It is a collection of both existing and new findings.
