...
###### Generate `DAST_PATHS_FILE` as an artifact

> [Introduced](https://gitlab.com/gitlab-org/gitlab/-/issues/273141) in GitLab 13.10.

You can save the `DAST_PATHS_FILE` as an artifact from a build that runs before DAST.
...
generate_url_file:
  stage: build
  script:
    - echo 'example_url.com' >> /url_file.txt
  artifacts:
    paths:
      - /url_file.txt
...
###### Save `DAST_PATHS_FILE` in the repository

> [Introduced](https://gitlab.com/gitlab-org/gitlab/-/issues/258825) in GitLab 13.6.

You can also save the `DAST_PATHS_FILE` in the repository. If a file matching `DAST_PATHS_FILE` is both
generated by a job and saved in the repository, the one generated by the job will take precedence.

By default, DAST scans do not clone the project repository. If the file is checked in to the project, instruct the DAST
job to clone the project by setting `GIT_STRATEGY` to fetch. The file is expected to be in the `/zap/wrk` directory.
