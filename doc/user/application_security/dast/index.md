...
1. Add it in an `environment_url.txt` file at the root of your project.
   ...
   an app dynamically created during a GitLab CI/CD pipeline, a job that runs prior to the DAST scan needs to 
   persist the app's domain in an `environment_url.txt` file. DAST will
   automatically parse the `environment_url.txt` file to find its scan target.

  For example, in a job that runs prior to DAST, you could include code that looks similiar to:
