...
###### Generating `DAST_PATHS_FILE` as an artifact

It is possible to save the `DAST_PATHS_FILE` as an artifact from a build that runs before DAST.
...
###### Saving `DAST_PATHS_FILE` in the repository

It is also possible to save the `DAST_PATHS_FILE` in the repository. If a file matching `DAST_PATHS_FILE` is both
