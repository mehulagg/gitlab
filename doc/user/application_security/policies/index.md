...
| `branches` | `array` of `string` | `*` or the branch's name | The branch the given policy applies to (supports wildcard). |
| `cadence` | `string` | *CRON expression:* ie. `0 0 * * *` | The string containing five fields separated by whitespace that is representing scheduled time. |
...
- name: Enforce DAST scan every 10 minutes
  description: This policy enforces DAST scan to be executed every 10 minutes
  enabled: true
  rules:
  - type: schedule
    branches:
...
NOTE:
When configuring policies with scheduled DAST scan the author of the commit in the repository of Security Policy Project must have access to
Scanner and Site Profiles, otherwise the scan will not be successfully scheduled.
