...
Using GitLab Group Migration, you can import existing groups from GitLab.com or self-managed instance. Group could be imported as sub-group of nay existing top level group. Only top-level groups migration is supported.

The following aspects of a group are imported:
...
Any other items are **not** exported.
...
Before you begin, ensure that target instance of GitLab (one initiating import) could reach remote Instance (one containing groups for export) via HTTP or HTTPS.

NOTE:
This might involve reconfiguring your firewall to prevent blocking connection on the side of self-managed instance.

### Connecting to remote GitLab instance
