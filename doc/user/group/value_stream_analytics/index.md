...
## Deploys and Deployment Frequency

The **Deploys** and **Deployment Frequency** metrics in Value Stream Analytics show how often you are delivering value to end users. A higher deployment frequency means you are getting improvements and features out faster, and able to get feedback and iterate more quickly. Both metrics show deployments for all projects in the group, unless you have selected specific projects in the **Select projects** dropdown. For the selected time period, the **Deploys** metric shows the total number of deployments to production. The **Deployment Frequency** metric shows the average number of deployments to production per day during the selected time period. Deployments must have a `finished_at` timestamp to be counted in these metrics. 
