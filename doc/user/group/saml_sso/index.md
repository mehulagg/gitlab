...
With this option enabled, users must go through your group's GitLab single sign-on URL if they wish to access project/group resources via the UI. They will still be able to login with username/email address and password but will be redirected to login via SAML if they are trying to access the group/project resources. They may also be added via SCIM, if configured but can't be added manually.
