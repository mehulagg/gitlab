...
The repository pushes shortly thereafter. To force a push, click the **Update now** (**{retry}**) button.
...
- Add the webhook URL which (in this case) uses the [Pull Mirror API](../../../api/projects.md#start-the-pull-mirroring-process-for-a-project) request to trigger an immediate pull after updates to the repository.
...
- `unknown_git` user is used as the commit author if the GitLab user does not exist in
  Perforce Helix.
