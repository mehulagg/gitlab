...
the GitLab Kubernetes Agent Server (KAS) is available on GitLab.com with address `wss://kas.gitlab.com`

In order to use the Kubernetes Agent, your GitLab instance needs to have the
Kubernetes Agent Server (KAS) installed.
...
- If you are a GitLab.com user, you can use `wss://kas.gitlab.com` as the `--kas-address` when you
