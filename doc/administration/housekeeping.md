...
- [`git gc`](https://git-scm.com/docs/git-gc) runs a number of housekeeping tasks such as:
  ...
  - Removing unreachable objects that may have been created from prior invocations of `git add`.
- [`git repack`](https://git-scm.com/docs/git-repack) re-organizes existing packs into a single,
  more efficient pack.
...
The following housekeeping optiojns are available:
...
- **Enable automatic repository housekeeping**: Regularly run `git repack` and `git gc`. If you
  ...
  slower and your repositories will use more disk space.
- **Enable Git pack file bitmap creation**: Create pack file bitmaps which accelerated `git clone`
...
- `pushes_since_gc` value is 50, a `repack -A -d --pack-kept-objects` runs.
