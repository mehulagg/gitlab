...
1. Follow the [instructions to migrate repmgr to Patroni](../../postgresql/replication_and_failover.html#switching-from-repmgr-to-patroni).
  - When configuring Patroni on each primary site database node, add
```ruby
patroni['replicaton_slots'] = { '<slot_name>' => 'physical' }
```
