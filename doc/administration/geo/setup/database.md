...
You will need the following information when setting up the database instances

#### PRIMARY_NODE_NAME

A **unique name** for the primary Geo instance
...
The plain text password for the `gitlab_replicator` user

#### PRIMARY_DB_LISTEN_ADDRESS

The IP address that the primary database will listen on
...
#### SECONDARY_DB_LISTEN_ADDRESS

The IP address that the secondary database will listen on
...
1. Configure PostgreSQL to listen on network interfaces:
   ...
   ##
   postgresql['listen_address'] = 'PRIMARY_DB_LISTEN_ADDRESS'

   ##
   # Allow PostgreSQL client authentication from the primary and secondary IPs. These IPs may be
   # public or VPC addresses in CIDR format, for example ['198.51.100.1/32', '198.51.100.2/32']
   ##
   postgresql['md5_auth_cidr_addresses'] = ['PRIMARY_DB_LISTEN_ADDRESS/32', 'SECONDARY_DB_LISTEN_ADDRESS/32']
...
1. Omnibus GitLab already has a [replication user](https://wiki.postgresql.org/wiki/Streaming_Replication)
   ...
   You will be prompted to enter a password. Make sure to enter REPLICATION_USER_PASSWORD:
