...
A production-ready and secure setup requires at least one internal load-balancing
node, three Consul nodes, three Patroni nodes on the primary site, and a similar
configuration for the secondary site except for the internal load-balancing node
that is not required. Be sure to use password credentials and other database
best practices.
