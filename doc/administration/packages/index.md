...
Optionally, track progress and verify all packages migrated.
...
(`sudo gitlab-psql -d gitlabhq_production` for Omnibus GitLab) verify `objectstg` below (where `file_store=2`) has count of all packages:
