...
| `gitlab_ci_difference_live_vs_actual_minutes` | Histogram   | 13.12    | Difference between CI minutes consumption counted while jobs were running (live) vs when job completed (actual). Used to enforce CI minutes consumption on long running jobs. | `plan` |
