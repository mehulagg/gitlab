...
When setting up Grafana through the process above, no scope shows in the
**Admin Area > Applications > GitLab Grafana** screen. However, the `read_user` scope is required
and it's provided to the application automatically. Note that setting any scope other than
...
Prior to GitLab version 13.10, the API scope is set and is required to access Grafana through the
GitLab OAuth provider. If you see the above error, make sure that one of the following is true in
the GitLab Grafana configuration screen:
