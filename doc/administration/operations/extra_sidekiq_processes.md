...
When `queue_selector` is set, all `queue_groups` must use [worker matching
query](#worker-matching-query).

### Example queue groups configuration
...
Opposed to [Queue selector](#queue-selector), which allows watching a set of
workers or queues when starting a Sidekiq cluster, GitLab also supports routing
a job from a worker to a desirable queue before it is scheduled. Sidekiq
clients try to match a job against a configured list of routing rules. Rules are
evaluated from first to last, and as soon as we find a match for a given worker
we stop processing for that worker (first match wins). If the worker doesn't
match any rule, it falls back the queue name generated from the worker name.
...
GitLab provides a simple expression syntax to match a worker based on its
attributes. This expression syntax is employed by both [Queue
routing rules](#queue-routing-rules) and [Queue selector](#queue-selector).
