...
- Direct access to [Gitaly](#gitaly) storage using repository storage paths, where each repository is stored on
  the assigned Gitaly node. All requests are routed to this node.
- Virtual storage provided by [Gitaly Cluster](#gitaly-cluster), where each repository is stored on
  multiple Gitaly nodes for fault tolerance. In a Gitaly Cluster:
...
benefit of using Gitaly Cluster virtual storage over direct Gitaly storage is:
...
Under some workloads, CPU and memory requirements may require a large fleet of Gitaly nodes. It can
be uneconomical to have one to one [replication factor](praefect.md#replication-factor).
...
In this configuration, every Git repository is stored on every Gitaly node in the cluster.
