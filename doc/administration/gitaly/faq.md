...
## Are their instructions for migrating to Gitaly Cluster?
...
there is a failover, Praefect isn't aware and start to fail as the database it's trying to use would
either:

- Be unavailable.
- In read-only mode.

In future solution may allow for Praefect and Omnibus GitLab databases on the same PostgreSQL server.
