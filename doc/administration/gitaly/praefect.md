...
1. Running two different election strategies side by side can cause a split brain, where different
   Praefect nodes consider repositories to have different primaries. This can be avoided by:

   1. If short downtime is acceptable:
   ...
   1. If downtime is unacceptable:
      ...
      on all of the Praefect nodes. This ensures there's only one Gitaly node configured, causing both of the election
      strategies to elect the same Gitaly node as the primary.

      1. Run `gitlab-ctl reconfigure` on all of the Prafect nodes. Wait until all of the Praefect processes have restarted and
      ...
      1. Uncomment the secondary Gitaly node configuration commented out in the earlier step on all of the Praefect nodes.

      1. Finally, run `gitlab-ctl reconfigure` on all of the Prafect nodes to reconfigure and restart the Praefect processes.
