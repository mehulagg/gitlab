...
Praefect regularly checks the health of each backend Gitaly node. This information can be used to
automatically failover to a newly-elected primary Gitaly node if the current primary node is found
to be unhealthy.
...
- A [Consul](../consul.md) primary election strategy.
- A cloud-native primary election strategy.
...
Gitaly primar nodes are enabled in `/etc/gitlab/gitlab.rb` by setting
`praefect['failover_election_strategy'] = 'per_repository'`.

Praefect's [deprecated election strategies](#deprecated-elections-strategies):
...
The `per_repository` election strategy solves this problem by electing a primary separately for each
...
1. Running two different election strategies side by side can cause a split brain, where different
   Praefect nodes consider repositories to have different primaries. To avoid this, all Praefect
   nodes should be shut down prior to changing the election strategy.
...
- **PostgreSQL:** Enabled by default until GitLab 14.0, and equivalent to:
  ...
  - Allows multiple Praefect nodes to coordinate via the PostgreSQL database to elect a primary
    Gitaly node.
  - Causes Praefect nodes to elect a new primary, monitor its health, and elect a new primary if the
    current one has not been reachable in 10 seconds by a majority of the Praefect nodes.
- **Memory:** Enabled by setting `praefect['failover_election_strategy'] = 'local'`
  in `/etc/gitlab/gitlab.rb` on the Praefect node.

  If a sufficient number of health
