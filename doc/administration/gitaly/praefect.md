...
WARNING:
The `reconcile` sub-command was deprecated in GitLab 13.12 is scheduled for removal in GitLab 14.0. Use
[the automatic reconciliation](#automatic-reconciliation) instead. The manual reconciliation may produce
excess replication jobs and is limited in functionality. The manual reconciliation does not work when
[repository-specific primary nodes](#repository-specific-primary-nodes) are enabled.
