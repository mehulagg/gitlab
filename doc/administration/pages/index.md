...
> [Introduced](https://gitlab.com/gitlab-org/omnibus-gitlab/-/issues/5577) in GitLab 13.5.

NOTE: **Note:**
We currently actively work on allowing to store GitLab Pages content in the object storage
instead of NFS. This work is not yet finished, but you can follow the progress in
[this issue](https://gitlab.com/groups/gitlab-org/-/epics/3901).

You can already set these settings, but pages content will **not** be moved to object storage yet.
...
| `remote_directory` | The bucket name where Pages sites content will be stored | |
...
1. Edit `/etc/gitlab/gitlab.rb` and add the following lines; replacing with
   ...
   ```ruby
   gitlab_rails['pages_state_object_store_enabled'] = true
   gitlab_rails['pages_state_object_store_remote_directory'] = "pages"
   gitlab_rails['pages_state_object_store_connection'] = {
     'provider' => 'AWS',
     'region' => 'eu-central-1',
