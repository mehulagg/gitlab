...
GitLab can use [LDAP configuration secrets](../auth/ldap/index.md#using-encrypted-credentials-core-only) read from an encrypted file. Because these files on encrypted on the
filesystem. The following Rake tasks are provided for updating the contents.
