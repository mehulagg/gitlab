...
repmgr is disabled automatically.
...
Both repmgr and Patroni are both supported and available. Patroni is the only option for PostgreSQL 12 clustering and
...
1. [Reconfigure Omnibus GitLab](../restart_gitlab.md#omnibus-gitlab-reconfigure) on the primary node.
   This will make it the leader. You can check this with:
...
- Upgrading PostgreSQL creates a new data directory with a new control data. From Patroni's perspective
  ...
  the cluster state, which is stored in Consul, is wiped out. Once the upgrade is completed Patroni
  is instructed to bootstrap a new cluster. **Note that this will change your _cluster ID_**.
...
#### Add a node as a standby server
  ...
  this causes a split. The old master has to be resynced from
...
- Trust only the specific database cluster nodes.
...
1. [Reconfigure GitLab](../restart_gitlab.md#omnibus-gitlab-reconfigure) which re-adds the user with the proper permissions.
