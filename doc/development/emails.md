...
1. Find the `incoming_email` section in `config/gitlab.yml`, enable the
   ...
   incoming_email:
     ...
     # The email address including the `%{key}` placeholder that will be replaced to reference the item being replied to. This `%{key}` should be included in its entirety within the email address and not replaced by another value (e.g. emailadress+%key@gmail).
