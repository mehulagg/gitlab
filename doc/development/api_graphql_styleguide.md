...
will bind the GraphQL enum values to the Rails enum definition, so if values are
ever added to the Rails enum then we know the GraphQL enum would reflect the change.
