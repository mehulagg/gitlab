...
File diffs are collapsed (but be expandable) if 100 files have already been rendered.
...
The diff is be persisted and expandable if the patch size doesn't
...
File diff are suppressed (technically different from collapsed, but behaves the same, and is expandable) if it has more than 5000 lines.
