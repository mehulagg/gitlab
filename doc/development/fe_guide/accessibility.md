...
If the element is a static one, such as an icon, we can enclose it in a button as it's traditionally focusable.

The following code snippet is a good example of an icon with a tooltip.

- It is automatically focusable, since it is a button.
- It is given an accessible name with `aria-label`, since it is a button with no text.
