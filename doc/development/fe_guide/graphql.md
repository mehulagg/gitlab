...
While the Apollo client has support for simple polling, it is possible that for performance reasons, you might need to use our Etag-based caching rather than hit the databse each time. The larger and more complex the query, the likelier it is that you should use this appraoch. If you are not sure if it applies to your situation, discuss with your backend engineer. 
