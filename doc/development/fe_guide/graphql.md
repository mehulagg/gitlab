...
recognize that priorities might not always line-up, and that for the sake of iteration and
...
So you will need to make sure your changes pass the validation if they are to be merged before the
...
[Mocking API response with local Apollo cache](#mocking-api-response-with-local-apollo-cache), which
...
`.eslintignore` file.
Bare in mind that any file listed in here will not be validated at all. So if you're only adding
fields to an existing query, prefer the `@client` directive approach so that the rest of the query
is still validated.
