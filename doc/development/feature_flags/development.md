...
Due to naming collision system does not allow to use feature flag under
the same name as a licensed feature name. This name collision creates confusion
and was [widely discussed and removed](https://gitlab.com/gitlab-org/gitlab/-/issues/259611).

This implies that for checking licensed features a dedicated feature flag
under a different name needs to be added to the codebase and explicitly checked.

If you want to check the licensed feature with a feature flag this needs to be done explictly:
look like:
...
push_frontend_feature_flag(:my_licensed_feature, type: :licensed) if project.feature_available?(:licensed_feature)
```
