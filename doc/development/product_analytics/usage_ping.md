...
1. The JSON payload is then [posted to the Versions application]( https://gitlab.com/gitlab-org/gitlab/-/blob/master/app/services/submit_usage_ping_service.rb#L20) (If a firewall exception is needed, the required URL is <https://version.gitlab.com/> where the hostname is `version.gitlab.com` and the protocol is `TCP` and the port number is `443`).
