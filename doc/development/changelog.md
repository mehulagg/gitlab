...
- A change that [removes a feature flag](feature_flags/index.md) **must** have a changelog entry (with `type` field being "other").
- A change that [removes a feature and its feature flag](feature_flags/index.md) **must** have a changelog entry (with `type` field being "removed").
