...
| Usage | Guidance |
|-|-|
| currently | <!-- vale gitlab.CurrentStatus = NO -->Do not use when talking about the product or its features. The documentation describes the product as it is today.<!-- vale gitlab.CurrentStatus = YES --> |
| first-person singular (I or me) | <!-- vale gitlab.FirstPerson = NO -->Do not use. Use **you**, **we**, or **us** instead.<!-- vale gitlab.FirstPerson = YES --><br>(Tested in [`FirstPerson.yml`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/doc/.vale/gitlab/FirstPerson.yml).) |
| future tense | When possible, use present tense instead. For example, use `after you execute this command, GitLab displays the result` instead of `after you execute this command, GitLab will display the result`. |
| high availability and HA | Do not use. Direct readers to the GitLab [reference architectures](../../../administration/reference_architectures/index.md) for information about configuring GitLab to have the performance needed for additional users over time. |
| jargon | Do not use. Define the term or [link to a definition](#links-to-external-documentation). |
| Latin abbreviations, like e.g., and i.e. | <!-- vale gitlab.LatinTerms = NO -->Do not use.<br><br>- Instead of **e.g.**, use **for example**, **such as**, **for instance**, or **like**.<br>- Instead of **i.e.**, use **that is**.<br>- Instead of **via**, use **with**, **through**, or **by using**.<br><br>(Tested in [`LatinTerms.yml`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/doc/.vale/gitlab/LatinTerms.yml).)<!-- vale gitlab.LatinTerms = YES --> |
...
| simply, easily, handy, useful | <!-- vale gitlab.Simplicity = NO -->Do not use. If the user doesn't find the process to be these things, we lose their trust.<!-- vale gitlab.Simplicity = YES --> |
| slashes | Instead of "and/or," use "or," or another sensible construction. This rule applies to other slashes as well, like follow/unfollow. Exception like CI/CD are allowed. |
