...
- All links present on the data file must end in `.html` (with the exception
  of `index.html` files), not `.md`.
...
- For `https://docs.gitlab.com/ee/README.html`, the global nav URL is `ee/README.html`.
- For `https://docs.gitlab.com/ee/user/analytics/value_stream_analytics.md`, the global nav
  URL is `ee/user/analytics/value_stream_analytics.html`.
- For `https://docs.gitlab.com/omnibus/settings/database.html`, the global nav URL is `omnibus/settings/database.html`.
- For `https://docs.gitlab.com/ee/install/index.html`, the global nav URL is `ee/install/`.
...
Depending on what documentation file you change, the [global nav URL](#urls) has a different prefix.
