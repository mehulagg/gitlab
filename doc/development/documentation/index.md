...
When moving or renaming a page, you must add redirect links to the new page. This
ensures users find the new page, and have the opportunity to update their bookmarks.
Redirect files added into the docs themselves are for users who view the docs in
`/help` on self-managed instances. Redirects in the `_redirect` file affect users
