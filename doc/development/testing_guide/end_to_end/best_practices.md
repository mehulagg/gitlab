...
The initial example uses the `have_job` matcher which is derived from the `has_job?` predicate method of the `Page::Project::Pipeline::Show` page object.
...
And then we could use either of the two now equivalent `expect` statements in the following example:
