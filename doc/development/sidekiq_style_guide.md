...
process icks up jobs for that worker too (after having been
...
A job scheduled for an idempotent worker is automatically
...
If the worker class is not marked as idempotent, a cop fails.
...
are passed along to the `perform_work` method.
...
In most cases, when scheduling a job from a request, this context is
