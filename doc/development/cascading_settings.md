...
## Displaying cascading settings on the frontend

There are a few reusable Rails view helpers, HAML partials, and JavaScript functions that can help display a cascading setting on the frontend.
...
Calls through to [`_locked?` method](#_locked-method) to check if the setting is locked.

#### Arguments

| Argument | Description | Type | Required (default value) |
|-|-|-|-|
| `attribute` | The name of the setting. e.g. `:delayed_project_removal`. | `String | Symbol` | `true` |
| `group` | The current group. | [`Group`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/app/models/group.rb) | `true` |
...
|-|-|-|-|
| `attribute` | The name of the setting. e.g. `:delayed_project_removal`. | `String | Symbol` | `true` |
| `form` | The [Rails FormBuilder object](https://apidock.com/rails/ActionView/Helpers/FormBuilder). | [`ActionView::Helpers::FormBuilder`](https://apidock.com/rails/ActionView/Helpers/FormBuilder) | `true` |
...
| `form` | The [Rails FormBuilder object](https://apidock.com/rails/ActionView/Helpers/FormBuilder). | [`ActionView::Helpers::FormBuilder`](https://apidock.com/rails/ActionView/Helpers/FormBuilder) | `true` |
| `setting_locked` | If the setting is locked. Can be calculated with [`cascading_namespace_setting_locked?`](https://gitlab.com/gitlab-org/gitlab/-/blob/c2736823b8e922e26fd35df4f0cd77019243c858/app/helpers/namespaces_helper.rb#L86). | `Boolean` | `true` |
| `settings_path_helper` | Lambda to generate a path to the ancestor group that locked the setting. e.g. `settings_path_helper: -> (locked_ancestor) { edit_group_path(locked_ancestor, anchor: 'js-permissions-settings') }` | `Lambda` | `false` (`false`) |
...
| `attribute` | The name of the setting. e.g. `:delayed_project_removal`. | `String | Symbol` | `true` |
| `setting_locked` | If the setting is locked. Can be calculated with [`cascading_namespace_setting_locked?`](https://gitlab.com/gitlab-org/gitlab/-/blob/c2736823b8e922e26fd35df4f0cd77019243c858/app/helpers/namespaces_helper.rb#L86). | `Boolean` | `true` |
| `settings_path_helper` | Lambda to generate a path to the ancestor group that locked the setting. e.g. `-> (locked_ancestor) { edit_group_path(locked_ancestor, anchor: 'js-permissions-settings') }` | `Lambda` | `false` (`false`) |
| `help_text` | Text shown below the checkbox. | `String` | `false` (`nil`) |
...
Initializes the JavaScript needed to display the popover when hovering over the lock icon. This function should be imported and called in the `app/assets/javascripts/pages` directory. 
...
= form_for @group do |f|
  ...
  .form-group{ data: { testid: 'delayed-project-removal-form-group' } }
    .gl-form-checkbox.custom-control.custom-checkbox
      ...
      = render 'shared/namespaces/cascading_settings/setting_label_checkbox', attribute: :delayed_project_removal,
          ...
          help_text: s_('GroupSettings|Projects will be permanently deleted after a 7-day delay. Inherited by subgroups.') do
  ...
  %fieldset
    = render 'shared/namespaces/cascading_settings/setting_label_fieldset', attribute: :merge_method,
        ...
        settings_path_helper: -> (locked_ancestor) { group_settings_repository_path(locked_ancestor, anchor: 'js-merge-method') },
    ...
    .form-group
      .gl-form-radio.custom-control.custom-radio
        = form.radio_button :merge_method, :merge, class: "custom-control-input", disabled: merge_method_locked
        = form.label :merge_method_merge, class: 'custom-control-label' do
          = s_('Settings|Merge commit')
          %p.help-text
            = s_('Settings|Every merge creates a merge commit.')

      .gl-form-radio.custom-control.custom-radio
        = form.radio_button :merge_method, :rebase_merge, class: "custom-control-input", disabled: merge_method_locked
        = form.label :merge_method_rebase_merge, class: 'custom-control-label' do
          = s_('Settings|Merge commit with semi-linear history')
          %p.help-text
            = s_('Settings|Every merge creates a merge commit.')

      .gl-form-radio.custom-control.custom-radio
        = form.radio_button :merge_method, :ff, class: "custom-control-input", disabled: merge_method_locked
        = form.label :merge_method_ff, class: 'custom-control-label' do
          = s_('Settings|Fast-forward merge')
          %p.help-text
            = s_('Settings|No merge commits are created.')
    .form-group
      = render 'shared/admin/cascading_settings/enforcement_checkbox', attribute: :merge_method,
          form: f,
          setting_locked: merge_method_locked
```
