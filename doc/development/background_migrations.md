...
1. Make sure to know how much time it'll take to run all scheduled migrations. Author should provide an estimation section explaining timings from the linked query plans and batches as discribed in the migration. For example, lets imagine a migration that deletes data, estimation section in MR description would look something as follows

```ruby
Estimated times 
900ms for select statement with 1000 items  (1000 is batch size)
3ms for delete statement with 1000 items
47600 items to delete
47600 / 1000 = 48 loops
2 mins delay per loop 
( 48 * 2 ) + (48 * 0.01505)  = ~100 mins to run the all scheduled batches
```
