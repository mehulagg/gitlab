...
This script may also be triggered via the `memory-gc-stats` job. When running locally, the script might execute faster by increasing
its parallelization using the `PAR` environment variable. For instance, `PAR=8` will attempt to always run 8 collections in parallel.
