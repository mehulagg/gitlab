...
In these situations GitLab follows a [deprecation process](#deprecation-process) where the deprecated
part of the schema is supported for a period of time before being removed.
...
1. Items meeting criteria are removed in `X.0` or `X.6` and added to the [removed items page](removed_items.md).
