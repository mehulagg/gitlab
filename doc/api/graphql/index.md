...
GraphQL mutations might be detected as spam.
If this happens a [GraphQL top-level error](https://spec.graphql.org/June2018/#sec-Errors) will be raised.

```json
{
  "errors": [
    {
      "message": "Request has been denied: SPAM detected",
...
In some cases, a mutation may be detected as potential spam.
In this case, the request can be resubmitted if a CAPTCHA service is configured.
The `captchaSiteKey` should be used to obtain a CAPTCHA response value via the appropriate CAPTCHA API
(currently only [Google reCAPTCHA v2](https://developers.google.com/recaptcha/docs/display) is supported).
Then the request can be resubmitted with the `X-GitLab-Captcha-Response` and `X-GitLab-Spam-Log-Id` headers set.
