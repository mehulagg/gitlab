...
Find a runner. Available only when feature flag `runner_graphql_query` is enabled (it is disabled by default).
...
Find runners visible to the current user. Available only when feature flag `runner_graphql_query` is enabled (it is disabled by default).
...
Available only when feature flag `custom_emoji` is enabled (it is disabled by default).
...
Available only when feature flag `runner_graphql_query` is enabled (it is disabled by default).
...
| <a id="groupcustomemoji"></a>`customEmoji` | [`CustomEmojiConnection`](#customemojiconnection) | Custom emoji within this namespace. Available only when feature flag `custom_emoji` is enabled (it is disabled by default). (see [Connections](#connections)) |
