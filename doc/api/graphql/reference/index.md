...
| Value | Description |
| ----- | ----------- |
| `CANCELED` | A build that is canceled |
| `CREATED` | A build that is created |
| `FAILED` | A build that is failed |
| `MANUAL` | A build that is manual |
| `PENDING` | A build that is pending |
| `PREPARING` | A build that is preparing |
| `RUNNING` | A build that is running |
| `SCHEDULED` | A build that is scheduled |
| `SKIPPED` | A build that is skipped |
| `SUCCESS` | A build that is success |
| `WAITING_FOR_RESOURCE` | A build that is waiting for resource |
