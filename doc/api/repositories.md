...
WARNING:
GitLab treats trailers case-sensitively. This means that if you set the
`trailer` field to `Example`, GitLab _won't_ include commits that use the
trailer `example`, `eXaMpLE`, or anything else that isn't _exactly_ `Example`.
