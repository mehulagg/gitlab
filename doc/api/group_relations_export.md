...
Group relations export allows you to partially export group structure. It is similar to Group Export, but it exports
each top level relation (e.g. milestones/boards/labels) as a separate file, instead of one archive. 
...
View status of relations export.
...
| --------- | -------------- | -------- | ---------------------------------------- |
| `id`      | integer/string | yes      | ID of the group owned by the authenticated user |
| `relation`      | string | yes      | Name of group top level relation to download |
