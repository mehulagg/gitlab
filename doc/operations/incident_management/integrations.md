...
Webhook URLs for Alert Integrations are read-only and are generated when the integration is saved
for the first time. It includes a name segment and a generated ID.

![Webhook URL](img/alert_integration_url_v13_10.png)

The name segment serves as a way to more easily distinguish multiple integrations externally, and is
ignored by the alert receiver. This means that if you update an integration name, alerts sent to the
previous endpoint continue to work.
