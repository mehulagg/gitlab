...
1. Clone the Gitaly repository. Replace `<X-Y-stable>` with the stable branch
   that matches the GitLab version you want to install. For example, if you
   want to install GitLab 13.6, use the branch name `13-6-stable`:

   ```shell
   git clone https://gitlab.com/gitlab-org/gitaly.git -b <X-Y-stable> /tmp/gitaly
   ```

1. Compile and install Gitaly's Git:
...
1. You can now remove the system Git and its dependencies:
