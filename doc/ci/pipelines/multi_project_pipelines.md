...
Global variables can be stopped from reaching the downstream pipeline by using the `inherit` keyword.
In the following configuration, the `MY_GLOBAL_VAR` variable is not available in the triggered pipeline.
