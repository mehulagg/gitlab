...
- Define cache globally or per job by using the `cache:` keyword. Otherwise it is disabled.
- If you define cache per job:
  - Subsequent pipelines can use it.
  - If the dependencies are identical, subsequent jobs in the same pipeline can use it.
- You cannot share cache between projects.
...
- Define artifacts per job by using the `artifacts:` keyword. Otherwise they are disabled.
- Artifacts can be used by subsequent jobs in the same pipeline only.
  (You can't use the same artifacts in multiple pipelines.)
- You can't use artifacts in a different job in the same stage.
- You can use artifacts in jobs in later stages of the same pipeline.
- Artifacts expire after 30 days unless you define an [expiration time](../yaml/README.md#artifactsexpire_in).
- Use [dependencies](../yaml/README.md#dependencies)
