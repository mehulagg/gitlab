...
- You can define it globally or per job by using the `cache:` keyword. Otherwise it is disabled.
...
- You can define artifacts per job by using the `artifacts:` keyword. Otherwise they are disabled.
- You can define it per job so that:
  - Subsequent jobs in the same pipeline can use it. (You can't use the same artifacts in multiple pipelines.)
  - Subsequent stages in the same pipeline can use it.
