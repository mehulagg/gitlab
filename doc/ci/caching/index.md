...
If you use cache and artifacts to store the same path in your jobs, the cache could
...
- `cache`: **For storing project dependencies**
  ...
  Caches are used to speed up runs of a given job in **subsequent pipelines**. You can
  ...
  It's possible to configure a cache to pass intermediate build results between stages,
  but this should be done with artifacts instead.

- `artifacts`: **Use for stage results that are passed between stages.**

  Artifacts are files generated by a job which are stored and uploaded. You can then
