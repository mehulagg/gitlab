...
* Use the `VAULT_CACERT` environment variable to configure GitLab Runner to trust the certificate.
  * If you are using systemd to manage GitLab Runner, please refer to this section for more information on how to add an environment variable for GitLab Runner: [Setting custom environment variables](https://docs.gitlab.com/runner/configuration/init.html#setting-custom-environment-variables)
  * If you deployed GitLab Runner using the [Helm chart](https://docs.gitlab.com/runner/install/kubernetes.html), you can do the following steps:
    1. Follow the steps listed here: [Providing a custom certificate for accessing GitLab](https://docs.gitlab.com/runner/install/kubernetes.html#providing-a-custom-certificate-for-accessing-gitlab).
      * Note: Add the certificate for the Vault server instead of the certificate for GitLab. If your GitLab instance is also using a self-signed certificate, you should be able to add both in the same `Secret`.
    2. Add these lines in your `values.yaml` file:
