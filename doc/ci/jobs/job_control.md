...
You can compare a variable to `null` keyword to see if it is defined. This expression
evaluates to true if variable is not defined when `==` is used, or to false if `!=` is used.
For example:
...
You can check for the presence of a variable by using just the variable name in
