...
You can't directly set values for the `POSTGRES_DB`, `POSTGRES_USER`,
`POSTGRES_PASSWORD` and `POSTGRES_HOST_AUTH_METHOD` variables using the GitLab
user interface. To set them,
[assign them to a variable in the user interface](../variables/README.md#create-a-custom-variable-in-the-ui),
and then assign that variable to the `POSTGRES_DB`, `POSTGRES_USER`,
`POSTGRES_PASSWORD` and `POSTGRES_HOST_AUTH_METHOD` variables in your
`.gitlab-ci.yml` file.
