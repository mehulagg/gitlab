...
- `if: '$CI_COMMIT_BRANCH == $CI_DEFAULT_BRANCH'`: If changes are pushed to the default
  branch (usually `master`). Use this if reusing the same configuration in multiple
...
The `only:changes` policy should only be used for pipelines triggered by the following
refs:
...
Use `environment` to define that a job deploys to a specific [environment](../environments/index.md).
...
You can't use variables defined under `script`.
...
as Review Apps. You can see an example using Review Apps at
...
CAUTION: **Caution:**
Be aware that semaphore_test_boosters reports usages statistics to the author.
