...
To skip artifact download for a job, you can define an empty array.
...
If you use a `trigger` with `when:manual` you receive a
`jobs:#{job-name} when should be on_success, on_failure or always` error.
This error occurs because `when:manual` prevents triggers being used.
