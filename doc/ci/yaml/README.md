...
To refer to the file, use a full path relative to the root directory (`/`).
...
You can also use a shorter local include to define the path:
...
To refer to the file, use a full path, relative to the root directory (`/`). For example:
...
Use `stage` to define which stage a job runs in. Jobs of the same
`stage` are executed in parallel (subject to [certain conditions](#use-your-own-runners)).

You define a `stage` for each job. You must also define [`stages`](#stages) globally.
For example:
...
Use `pre` and `post` to make a stage be the first or last one in a pipeline.
...
You must create a job in at least one stage other than `.pre` or `.post`.
...
- If the `if` statement is true, the job is included in the pipeline. This behavior is the default.
- If the `if` statement if false, the job is not included in the pipeline. This is the behavior when you use `when: never`.
...
Use `exists` to determine if files exist at a specific repository path.
...
You must specify the Docker image to use for the `release-cli`. Use the following directive:
...
The `tag_name` can refer to an existing Git tag or can be specified by the user. It is required.
...
is then published as a website.

- You must place any static content under a `public/` directory.
- You must define `artifacts` with a path to the `public/` directory.
...
Use `inherit:` to disable inheritance of globally-defined defaults
and variables.
...
use it instead of these YAML features.
