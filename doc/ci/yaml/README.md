...
Be careful not to have rules that match both branch pipelines and merge request pipelines,
which can cause [duplicate pipelines](#prevent-duplicate-pipelines).
...
You can also have template files stored in a central repository that projects can `include`.
...
`before_script` is used to define and array of commands that should run before each job,
but after the restoration of any [artifacts](#artifacts).
...
job1:
  script:
    - echo "This scrip executes after the global before_script."
...
`after_script` is used to define and array of commands that run after each job,
...
You can configure jobs to use `only: changes` with other `only: refs` keywords, but
those jobs always run, ignoring the changes.
...
You can run some jobs without waiting for others to complete, ignoring stage ordering.
...
`pow`, this environment would be accessible with an URL like `https://review-pow.example.com/`.
