...
- The `variables` sections are merged, but `URL: "http://my-url.internal"`
  is overwritten by `URL: "http://docker-url.internal"`.
- `tags: ['production']` is overwritten by `tags: ['docker']`.
- `script` is not merged, but `script: ['echo "Hello world!"']` is
  overwritten by `script: ['rake rspec']`. Arrays can be
...
When there is a delayed job in a stage, the pipeline doesn't progress until the delayed job finishes.
...
`tags: [dev]` was overwritten by `tags: [postgres]`.
