...
1. Downloads any missing large files needed to checkout the `master` branch.
...
For repositories with millions of files and long history, you can exclude all files and use with
`sparse-checkout` to reduce the size of your working copy.
...
`.gitignore` file to specify which files should be included when cloning and fetching.
...
1. Create a new Git repository and clone:
