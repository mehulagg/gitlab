    ...
    module References
      ...
      class AbstractReferenceFilter < ReferenceFilter
        ...
        def initialize(doc, context = nil, result = nil)
          ...
          @cache     = ReferenceCache.new(object_class, parent_type, parent, self, context)
          @cache_two = ReferenceCacheTwo.new(object_class, parent_type, parent, self, context)
        end
        ...
        def call
          ...
          binding.pry
          @cache_two.load_references_per_parent(nodes)
        ...
        def object_link_filter(text, pattern, link_content: nil, link_reference: false)
          references_in(text, pattern) do |match, id, project_ref, namespace_ref, matches|
            parent_path = if parent_type == :group
                            @cache.full_group_path(namespace_ref)
                          else
                            @cache.full_project_path(namespace_ref, project_ref)
                          end
