  ...
  class ApplicationRateLimiter
    class << self
      ...
      def throttled?(key, **options)
        ...
        scoped_user = fetch_scoped_user(options[:scope])
        return if excluded_user?(scoped_user, options[:users_allowlist])
      ...
      def fetch_scoped_user(scope)
        scope.find { |s| s.is_a?(User) }
      end

      def excluded_user?(user, allowlist)
        return unless user && allowlist

        user.username.downcase.in?(allowlist.map(&:downcase))
      end
    end
  end
end
