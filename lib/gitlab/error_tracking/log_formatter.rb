  ...
  module ErrorTracking
    class LogFormatter
      ...
      def append_user_to_log!(payload, context_payload)
        user_context = Sentry.context.user.merge(context_payload[:user])
      ...
      def append_tags_to_log!(payload, context_payload)
        tags_context = Sentry.context.tags.merge(context_payload[:tags])
      ...
      def append_extra_to_log!(payload, context_payload)
        extra = Sentry.context.extra.merge(context_payload[:extra])
