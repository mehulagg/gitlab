      ...
      class << self
        ...
        def call(name, hooks)
          ...
          hooks.each do |hook|
            ...
            Gitlab::ErrorTracking.track_and_raise_for_dev_exception(e, type: 'LifecycleEvents', hook: hook)
            Gitlab::AppLogger.warn("WARNING hook #{name} failed with exception (#{e.class}) #{e.message}")

            # we consider lifecycle hooks to be fatal errors
            raise FatalError, e.message if USE_FATAL_LIFECYCLE_EVENTS
          end
        end
