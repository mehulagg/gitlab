...
module Gitlab
  module Cache
    module Helpers
      ...
      # @return [String]
      def cached_object(object, presenter:, presenter_args:, context:, expires_in:, dump: true)
        cache.fetch(contextual_cache_key(presenter, object, context), expires_in: expires_in) do
          presenter_json = presenter.represent(object, **presenter_args).as_json

          if dump
            Gitlab::Json.dump(presenter_json)
          else
            presenter_json
          end
        end
      ...
      # @param expires_in [ActiveSupport::Duration, Integer] an expiry time for the cache entry
      # @return [Array<String>]
      def cached_collection(collection, presenter:, presenter_args:, context:, expires_in:, dump: true)
        json = fetch_multi(presenter, collection, context: context, expires_in: expires_in) do |obj|
          presenter_json = presenter.represent(obj, **presenter_args).as_json
