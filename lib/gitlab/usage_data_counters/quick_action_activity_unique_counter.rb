    ...
    module QuickActionActivityUniqueCounter
      class << self
        ...
        def track_unique_action(name, args:, user:)
          return unless Feature.enabled?(:usage_data_track_quickactions)
          return unless user

          name = prepare_name(name, args)
          binding.pry

          Gitlab::UsageDataCounters::HLLRedisCounter.track_event(:"i_quickactions_#{name}", values: user.id, time: Time.zone.now)
        end
