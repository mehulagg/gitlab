    ...
    module RenameTableHelpers
      def rename_table_safely(old_table_name, new_table_name)
        with_lock_retries do
          ...
          execute("INSERT INTO renamed_tables (old_name, new_name) VALUES ('#{old_table_name}', '#{new_table_name}')")
        end
      end
      ...
      def undo_rename_table_safely(old_table_name, new_table_name)
        with_lock_retries do
          ...
          execute("DELETE FROM renamed_tables WHERE new_name = '#{new_table_name}'")
