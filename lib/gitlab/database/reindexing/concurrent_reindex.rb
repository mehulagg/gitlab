    ...
    module Reindexing
      class ConcurrentReindex
        ...
        def replacement_index_name
          @replacement_index_name ||= temporary_name(TEMPORARY_INDEX_PREFIX, index.name)
        end

        def replaced_index_name
          @replaced_index_name ||= temporary_name(REPLACED_INDEX_PREFIX, index.name)
        end

        def temporary_name(prefix, suffix)
          "#{prefix}#{Digest::SHA2.hexdigest(suffix)}".slice(0, PG_IDENTIFIER_LENGTH)
        end
