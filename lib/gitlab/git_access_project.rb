...
module Gitlab
  class GitAccessProject < GitAccess
    ...
    def namespace_path
      strong_memoize(:namespace_path) { project_path_match[:namespace_path] }
    end

    def project_path
      strong_memoize(:project_path) { project_path_match[:project_path] }
    end

    def project_path_match
      strong_memoize(:project_path_match) { repository_path.match(Gitlab::PathRegex.full_project_git_path_regex) || {} }
    end
