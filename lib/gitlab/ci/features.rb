...
module Gitlab
  module Ci
    ...
    module Features
      class << self
        def artifacts_exclude_enabled?
          ::Feature.enabled?(:ci_artifacts_exclude, default_enabled: true)
        end

        def instance_variables_ui_enabled?
        ...
        def pipeline_status_omit_commit_sha_in_cache_key?(project)
        ...
        def store_pipeline_messages?(project)
          ::Feature.enabled?(:ci_store_pipeline_messages, project, default_enabled: true)
        end

        def raise_job_rules_without_workflow_rules_warning?
        ...
        def disallow_to_create_merge_request_pipelines_in_target_project?(target_project)
          ::Feature.enabled?(:ci_disallow_to_create_merge_request_pipelines_in_target_project, target_project)
        end

        def project_transactionless_destroy?(project)
        ...
        def display_quality_on_mr_diff?(project)
