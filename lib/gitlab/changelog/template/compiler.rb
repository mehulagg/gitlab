    ...
    module Template
      ...
      class Compiler
        ...
        def compile(template)
          ...
          template.each_line { |line| erb << transform(line) }
          Template.new(erb)
        end
        ...
        def transform(line)
          line
            ...
            .gsub(IF) { "<% if truthy?(#{read_path(Regexp.last_match(1))}) -%>" }
            .gsub(VAR) { "<%= #{read_path(Regexp.last_match(1))} %>" }
        end
        ...
        def read_path(path)
          ...
          args = path
            .split('.')
            .map { |arg| arg.match?(INTEGER) ? "#{arg}" : "'#{arg}'" }
