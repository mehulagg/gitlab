...
module Gitlab
  module ImportExport
    class DecompressedArchiveSizeValidator
      ...
      CHUNK_SIZE = 16.megabytes.freeze
      ...
      def validate
        until archive_file.eof?
          ...
          inflate_stream.inflate(compressed_chunk) do |chunk|
            ...
            @total_inflated_chunks += 1

            # Start garbage collection every 32k inflated chunks in order
            # to prevent memory bloat during archive decompression.
            # Each inflated chunk is ~16Kb, which makes GC to start
            # each ~512Mb of inflated data.
      ...
      def gc_start?
        @total_inflated_chunks % @denominator == 0
      end
