...
namespace :gitlab do
  namespace :pages do
    ...
    task clean_migrated_zip_storage: :gitlab_environment do
      total_deployments = ::PagesDeployment.migrated_from_legacy_storage.count
      destroyed_deployments = 0

      logger.info("Starting to delete migrated pages deployments")
      logger.info("- #{total_deployments} deployments needs to be deleted")
      ...
      ::PagesDeployment.migrated_from_legacy_storage.each_batch(of: batch_size) do |batch|
        ...
        logger.info("#{destroyed_deployments}/#{total_deployments} deployments were deleted")
      end
    end
