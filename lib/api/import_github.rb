...
module API
  class ImportGithub < Grape::API::Instance
    ...
    helpers do
      def client
        @client ||= if Feature.enabled?(:remove_legacy_github_client)
                      Gitlab::GithubImport::Client.new(params[:personal_access_token], host: params[:github_host])
                    else
    ...
    params do
      ...
      optional :github_host, type: String, desc: 'GitHub Enterprise Host'
    end
