...
module Tooling
  module Danger
    module Changelog
      ...
      ChangelogCheckResult = Struct.new(:errors, :warnings, :markdowns, :messages, keyword_init: true)
      ...
      def check_changelog_trailer(commit)
        ...
        return ChangelogCheckResult.new(errors: []) if CATEGORIES.include?(category)

        ChangelogCheckResult.new(errors: ["Commit #{commit.sha} uses an invalid changelog category: #{category}"])
      end

      def check_changelog_yaml
        check_result = ChangelogCheckResult.new(errors: [], warnings: [], markdowns: [], messages: [])
        return check_result unless present?

        raw_file = File.read(changelog_path)
        yaml = YAML.safe_load(raw_file)
        yaml_merge_request = yaml["merge_request"].to_s

        check_result.errors << "`title` should be set, in #{helper.html_link(changelog_path)}! #{SEE_DOC}" if yaml["title"].nil?
        check_result.errors << "`type` should be set, in #{helper.html_link(changelog_path)}! #{SEE_DOC}" if yaml["type"].nil?
        ...
        ChangelogCheckResult.new(errors: ["#{helper.html_link(changelog_path)} isn't valid YAML! #{SEE_DOC}"])
      rescue StandardError => e
        ChangelogCheckResult.new(warnings: ["There was a problem trying to check the Changelog. Exception: #{e.class.name} - #{e.message}"])
      end
      ...
      def check_changelog_yaml_merge_request(raw_file:, yaml_merge_request:, check_result:)
        ...
        if yaml_merge_request.empty?
          ...
          if mr_line
            check_result.markdowns << { msg: format(SUGGEST_MR_COMMENT, mr_iid: helper.mr_iid), file: changelog_path, line: mr_line.succ }
          else
            check_result.messages << "Consider setting `merge_request` to #{helper.mr_iid} in #{helper.html_link(changelog_path)}. #{SEE_DOC}"
          end
        elsif yaml_merge_request != helper.mr_iid && !cherry_pick_against_stable_branch
          check_result.errors << "Merge request ID was not set to #{helper.mr_iid}! #{SEE_DOC}"
        end
      ...
      def check_changelog_path
        ...
        if ee_changes.any? && !ee_changelog? && !required?
          check_result.warnings << "This MR has a Changelog file outside `ee/`, but code changes in `ee/`. Consider moving the Changelog file into `ee/`."
        end

        if ee_changes.empty? && ee_changelog?
          check_result.warnings << "This MR has a Changelog file in `ee/`, but no code changes in `ee/`. Consider moving the Changelog file outside `ee/`."
        end

        if ee_changes.any? && ee_changelog? && required_reasons.include?(:db_changes)
          check_result.warnings << "This MR has a Changelog file inside `ee/`, but there are database changes which [requires](https://docs.gitlab.com/ee/development/changelog.html#what-warrants-a-changelog-entry) the Changelog placement to be outside of `ee/`. Consider moving the Changelog file outside `ee/`."
        end
