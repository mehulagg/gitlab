...
CREATE VIEW postgres_index_bloat_estimates AS
 SELECT relation_stats.nspname AS schemaname,
    ...
    (((100)::double precision * ((relation_stats.relpages)::double precision - relation_stats.est_pages)) / (relation_stats.relpages)::double precision) AS extra_ratio,
    relation_stats.fillfactor,
        ...
        END AS bloat_size,
    (((100)::double precision * ((relation_stats.relpages)::double precision - relation_stats.est_pages_ff)) / (relation_stats.relpages)::double precision) AS bloat_ratio,
...
END AS attrelname
  ...
  WHERE (relation_stats.nspname = ANY (ARRAY["current_schema"(), 'gitlab_partitions_dynamic'::name, 'gitlab_partitions_static'::name]))
