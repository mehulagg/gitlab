...
class AddSecretKeyAndIvToDastSiteProfile < ActiveRecord::Migration[6.0]
  ...
  def change
    ...
    add_column :dast_site_profiles, :encrypted_secret_key_iv, :text
    add_column :dast_site_profiles, :encrypted_secret_key_salt, :text

    add_column :dast_site_profiles, :secret_key_iv, :text # rubocop:disable Migration/AddLimitToTextColumns
  end
end
