...
class RenameIndexesOnGitLabCom < ActiveRecord::Migration[6.0]
  ...
  def up
    ...
    upgrade_to_primary_key(:schema_migrations, :version, 'schema_migrations_version_key', 'schema_migrations_key')
  end
  ...
  def replace_unique_constraint_with_index(table, columns, old_name, new_name)
    ...
    execute "ALTER TABLE emails DROP CONSTRAINT #{quote_table_name(old_name)}"
  end
