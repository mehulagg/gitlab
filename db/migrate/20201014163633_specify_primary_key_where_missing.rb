...
class SpecifyPrimaryKeyWhereMissing < ActiveRecord::Migration[6.0]
  ...
  def up
    tables.each do |table, (unique_index, _)|
      with_lock_retries do
        execute "ALTER TABLE #{table} ADD CONSTRAINT #{table}_pkey PRIMARY KEY USING INDEX #{unique_index}"
      end
    end
  end
  ...
  def tables
    # TODO: check all columns are NOT NULL
    @tables ||= {
      ...
      elasticsearch_indexed_namespaces: [:index_elasticsearch_indexed_namespaces_on_namespace_id, %i(namespace_id)],
      elasticsearch_indexed_projects: [:index_elasticsearch_indexed_projects_on_project_id, %i(project_id)],
