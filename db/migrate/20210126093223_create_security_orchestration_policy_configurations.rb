...
class CreateSecurityOrchestrationPolicyConfigurations < ActiveRecord::Migration[6.0]
  ...
  def up
    ...
    create_table_with_constraints :security_orchestration_policy_configurations, comment: table_comment.to_json do |t|
      t.references :project, null: true, foreign_key: { to_table: :projects, on_delete: :cascade }, index: { name: INDEX_PREFIX + 'on_project_id' }
      t.references :security_policy_management_project, null: false, foreign_key: { to_table: :projects, on_delete: :restrict }, index: { name: INDEX_PREFIX + 'on_security_policy_management_project_id' }
  ...
  def down
    with_lock_retries do
      drop_table :security_orchestration_policy_configurations
    end
  end
end
