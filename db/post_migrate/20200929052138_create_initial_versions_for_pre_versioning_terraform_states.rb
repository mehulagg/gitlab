...
class CreateInitialVersionsForPreVersioningTerraformStates < ActiveRecord::Migration[6.0]
  ...
  def up
    execute <<-SQL
      INSERT INTO terraform_state_versions (terraform_state_id, created_by_user_id, created_at, updated_at, version, file_store, file)
      SELECT id, locked_by_user_id, NOW(), NOW(), 0, file_store, file
