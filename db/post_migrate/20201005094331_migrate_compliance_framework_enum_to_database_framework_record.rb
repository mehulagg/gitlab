...
class MigrateComplianceFrameworkEnumToDatabaseFrameworkRecord < ActiveRecord::Migration[6.0]
  ...
  class TmpProjectSettings < ActiveRecord::Base
    # Maps data between ComplianceManagement::ComplianceFrameworku::FRAMEWORKS(enum) and new ComplianceManagement::Framework model
    ...
    def raw_compliance_framework
      ...
      ENUM_FRAMEWORK_MAPPING.fetch(framework).merge(group_id: root_group_id)
    end
  end
  ...
  def up
    ...
    # This is our standard recursive namespace query, we use it to determine the root_group_id in the same query.
    lateral_join = <<~SQL
      INNER JOIN LATERAL (
        WITH RECURSIVE "base_and_ancestors" AS (
          (
            SELECT "ns".* FROM "namespaces" as ns WHERE "ns"."type" = 'Group' AND "ns"."id" = projects.namespace_id
          ) UNION
          (
             SELECT "ns".* FROM "namespaces" as ns, "base_and_ancestors" WHERE "ns"."type" = 'Group' AND "ns"."id" = "base_and_ancestors"."parent_id"
          )
    ...
    TmpProjectSettings.each_batch(of: 100) do |query|
      project_settings_with_root_group = query
        .select(:project_id, :framework, 'root_namespaces.id as root_group_id')
      ...
      ActiveRecord::Base.transaction do
        ...
        TmpComplianceFramework.insert_all(raw_frameworks) # Create compliance frameworks per group

        unique_group_ids = project_settings_with_root_group.map(&:root_group_id).uniq

        framework_records = TmpComplianceFramework.select(:id, :group_id, :name).where(group_id: unique_group_ids)
        ...
        project_settings_with_root_group.each do |project_setting|
          framework = framework_records.find do |record|
            ...
            record.name == project_setting.raw_compliance_framework[:name] && record[:group_id] == project_setting.raw_compliance_framework[:group_id]
          end
