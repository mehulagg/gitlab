...
class RetryBackfillTraversalIdsChildren < ActiveRecord::Migration[6.1]
  MIGRATION = 'BackfillNamespaceTraversalIdsChildren'
  ...
  def up
    ...
    bg_migration_job_class.where(class_name: MIGRATION, status: bg_migration_job_class.statuses['pending']).each do |job|
      Gitlab::BackgroundMigration::BackfillNamespaceTraversalIdsChildren.new.perform(*job.arguments)
    end
