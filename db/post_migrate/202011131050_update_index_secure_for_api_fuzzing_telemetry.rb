...
class UpdateIndexSecureForApiFuzzingTelemetry < ActiveRecord::Migration[6.0]
  ...
  NEW_SECURE_INDEX_NAME = 'index_secure_ci_builds_on_user_id_created_at'

  def up
    add_concurrent_index(:ci_builds,
                         [:user_id, :created_at],
                         where: "(((type)::text = 'Ci::Build'::text) AND ((name)::text = ANY (ARRAY[('container_scanning'::character varying)::text, ('dast'::character varying)::text, ('dependency_scanning'::character varying)::text, ('license_management'::character varying)::text, ('license_scanning'::character varying)::text, ('sast'::character varying)::text, ('coverage_fuzzing'::character varying)::text, ('api_fuzzing'::character varying)::text, ('api_fuzzing_dnd'::character varying)::text, ('secret_detection'::character varying)::text])))",
