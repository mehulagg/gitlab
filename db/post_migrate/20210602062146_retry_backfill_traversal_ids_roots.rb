...
class RetryBackfillTraversalIdsRoots < ActiveRecord::Migration[6.1]
  MIGRATION = 'BackfillNamespaceTraversalIdsRoots'
  ...
  def up
    ...
    bg_migration_job_class.where(class_name: MIGRATION, status: bg_migration_job_class.statuses['pending']).each do |job|
      Gitlab::BackgroundMigration::BackfillNamespaceTraversalIdsRoots.new.perform(*job.arguments)
    end
