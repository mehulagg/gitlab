  ...
  context 'one argument given' do
    ...
    it 'flags the use with a dynamic feature as nil' do
      expect_offense(<<~SOURCE)
        ...
        ^^^^^^^^^^^^^^^^^^^^^^^ `feature_available?` should not be called for features that can be licensed (`nil` given), use `licensed_feature_available?(feature, user)` instead.
      SOURCE
      expect_offense(<<~SOURCE)
    ...
    it 'flags the use with a feature that is not a project feature' do
      expect_offense(<<~SOURCE)
        ...
        ^^^^^^^^^^^^^^^^^^^^^^^^ `feature_available?` should not be called for features that can be licensed (`:foo` given), use `licensed_feature_available?(feature, user)` instead.
      SOURCE
      expect_offense(<<~SOURCE)
      ...
      expect_offense(<<~SOURCE)
        feature_available?(foo)
        ^^^^^^^^^^^^^^^^^^^^^^^ `feature_available?` should not be called for features that can be licensed (`foo` isn't a literal so we cannot say if it's legit or not), using `licensed_feature_available?(feature, user)` may be more appropriate.
      SOURCE
      expect_offense(<<~SOURCE)
        foo = :feature
        feature_available?(foo)
