...
RSpec.describe WhatsNewHelper do
  describe '#whats_new_storage_key' do
    ...
    context 'when version exist' do
      ...
      before do
        allow(ReleaseHighlight).to receive(:most_recent).and_return({ items: [release_item] })
    ...
    context 'when most recent release highlights do NOT exist' do
      before do
        allow(ReleaseHighlight).to receive(:most_recent).and_return(nil)
      end
  ...
  describe '#whats_new_most_recent_release_items_count' do
    ...
    context 'when recent release items exist' do
      it 'returns the count from the most recent file' do
        allow(ReleaseHighlight).to receive(:most_recent).and_return({ items: [double(:item)] })
