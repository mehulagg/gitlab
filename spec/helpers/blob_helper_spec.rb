...
RSpec.describe BlobHelper do
  ...
  describe '#ide_merge_request_path' do
    ...
    it 'returns IDE path for the given MR if MR is not merged' do
      expect(helper.ide_merge_request_path(merge_request)).to eq("/-/ide/project/#{project.full_path}/merge_requests/#{merge_request.id}")
    end
    ...
    context 'when the MR is merged' do
      let(:merge_request) { create(:merge_request, :merged, source_project: project, source_branch: 'testing-1', target_branch: 'feature-1') }
      ...
      context 'when target branch exists' do
        before do
          allow(merge_request).to receive(:target_branch_exists?) { true }
        end
