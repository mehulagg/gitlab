...
module DnsHelpers
  ...
  def stub_all_dns!
    ...
    allow(Addrinfo).to receive(:getaddrinfo).with(anything, anything, nil, :STREAM, timeout: kind_of(Integer)).and_return([])
  ...
  def stub_invalid_dns!
    ...
    allow(Addrinfo).to receive(:getaddrinfo).with(/\Afoobar\.\w|(\d{1,3}\.){4,}\d{1,3}\z/i, anything, nil, :STREAM, timeout: kind_of(Integer)) do
      raise SocketError.new("getaddrinfo: Name or service not known")
    end
  end
  ...
  def permit_local_dns!
    ...
    allow(Addrinfo).to receive(:getaddrinfo).with(local_addresses, anything, nil, :STREAM, timeout: kind_of(Integer)).and_call_original
