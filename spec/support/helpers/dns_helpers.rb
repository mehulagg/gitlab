...
module DnsHelpers
  ...
  def stub_all_dns!
    ...
    allow(Addrinfo).to receive(:getaddrinfo).with(anything, anything, nil, :STREAM, anything, anything, any_args).and_return([])
  end
  ...
  def stub_invalid_dns!
    ...
    allow(Addrinfo).to receive(:getaddrinfo).with(/\Afoobar\.\w|(\d{1,3}\.){4,}\d{1,3}\z/i, anything, nil, :STREAM, anything, anything, any_args) do
      raise SocketError.new("getaddrinfo: Name or service not known")
    end
  end
