  ...
  context 'changing state' do
    ...
    context 'to reopened' do
      it 'executes hooks only once' do
        described_class.new(container: project, current_user: user, params: { state_event: 'reopen' }).execute(closed_issuable)
      end
    end

    context 'to closed' do
      it 'executes hooks only once' do
        described_class.new(container: project, current_user: user, params: { state_event: 'close' }).execute(open_issuable)
      end
    end
  end
end
