...
RSpec.shared_examples 'creating the namespace package setting' do
  ...
  it 'saves the settings' do
    ...
    expect(namespace.package_setting.maven_duplicates_allowed).to eq(package_settings[:maven_duplicates_allowed])
    expect(namespace.package_setting.maven_duplicate_exception_regex).to eq(package_settings[:maven_duplicate_exception_regex])
  end
