...
RSpec.shared_examples 'deduplicate job strategy' do |strategy_class, name|
  let(:fake_duplicate_job) do
    instance_double(Gitlab::SidekiqMiddleware::DuplicateJobs::DuplicateJob)
  end

  subject(:strategy) { strategy_class.new(fake_duplicate_job) }
  ...
  describe '#schedule' do
    ...
    context "when the job is droppable" do
      ...
      it 'logs that the job was dropped' do
        ...
        expect(fake_logger).to receive(:log).with(a_hash_including({ 'jid' => 'new jid' }), "dropped #{name}", {})
      ...
      it 'logs the deduplication options of the worker' do
        ...
        expect(fake_logger).to receive(:log).with(a_hash_including({ 'jid' => 'new jid' }), "dropped #{name}", { foo: :bar })
