  ...
  describe '#resolve' do
    ...
    context 'when the user can create an issue' do
      ...
      it 'creates issue with correct values' do
        ...
        expect(issue.labels.pluck(:title)).to eq([project_label1.title, project_label2.title, new_label1, new_label2])
        expect(issue.assignees.pluck(:id)).to eq([assignee1.id, assignee2.id])
      end
      ...
      context 'when passing in label_ids' do
        ...
        it 'creates issue with correct values' do
          ...
          expect(issue).to have_attributes(expected_attributes)
          expect(issue.milestone_id).to eq(milestone.id)
          expect(issue.labels.pluck(:title)).to eq([project_label1.title, project_label2.title])
          expect(issue.assignees.pluck(:id)).to eq([assignee1.id, assignee2.id])
        end
      end

      context 'when trying to create with owner and admin only params' do
        before do
        ...
        it 'ignores the special params' do
          issue = subject[:issue]
          expect(issue).not_to be_like_time(special_params[:created_at])
          expect(issue.iid).not_to eq(special_params[:iid])
        end
      end
    end
    ...
    context 'when creating an issue as owner' do
      ...
      it 'ignores sets the special params' do
        issue = subject[:issue]
        expect(issue.created_at).to be_like_time(special_params[:created_at])
        expect(issue.iid).to eq(special_params[:iid])
      end
    end
  end
