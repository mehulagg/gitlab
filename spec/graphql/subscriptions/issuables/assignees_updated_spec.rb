...
RSpec.describe Subscriptions::Issuables::AssigneesUpdated do
  ...
  it { expect(described_class.payload_type).to eq(Types::Issuables::UpdatedAssigneesType) }
  ...
  describe '#resolve' do
    ...
    context 'subscription updates' do
      let_it_be(:object) { { assignees: [ create(:user) ] } }
      let(:resolver) { resolver_instance(described_class, obj: object, ctx: { current_user: current_user }, subscription_update: true) }
