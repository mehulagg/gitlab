...
RSpec.describe AddHasExternalIssueTrackerTrigger do
  ...
  describe '#up' do
    ...
    describe 'INSERT trigger' do
      ...
      it 'does not set `has_external_issue_tracker` to true when active other service is inserted' do
        expect do
    ...
    describe 'UPDATE trigger' do
      ...
      it 'does not change `has_external_issue_tracker` when `issue_tracker` is made inactive, if an active `issue_tracker` still exists' do
        services.create!(category: 'issue_tracker', active: true, project_id: @project.id)
        service = services.create!(category: 'issue_tracker', active: true, project_id: @project.id)
    ...
    describe 'DELETE trigger' do
      ...
      it 'does not set `has_external_issue_tracker` to false when `issue_tracker` is deleted, if an active `issue_tracker` still exists' do
        services.create!(category: 'issue_tracker', active: true, project_id: @project.id)
        service = services.create!(category: 'issue_tracker', active: true, project_id: @project.id)
