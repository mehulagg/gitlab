  ...
  it 'steals remaining jobs' do
    ...
    expect_next_instance_of(Gitlab::BackgroundMigration::Logger) do |logger|
      expect(logger).to receive(:info)
        .with(message: "#{described_class}: Project IDs with container_registry_enabled not copied by background migration were copied now: [#{project_feature4.id}, #{project_feature5.id}, #{project_feature6.id}]")
    end
