...
RSpec.describe ReleaseHighlight, :clean_gitlab_redis_cache do
  ...
  describe '.most_recent_version_digest' do
    ...
    it 'uses process memory cache' do
      expect(Gitlab::ProcessMemoryCache.cache_backend).to receive(:fetch).with("release_highlight:most_recent_version:#{Gitlab.revision}", expires_in: described_class::CACHE_DURATION)
