...
RSpec.describe Discussion do
  ...
  describe '#cache_key' do
    ...
    it 'returns the cache key' do
      expect(subject.cache_key).to eq("#{described_class::CACHE_VERSION}:#{third_note.latest_cached_markdown_version}:#{subject.id}:#{notes_sha}:")
    end
    ...
    context 'when discussion is resolved' do
      ...
      it 'returns the cache key with resolved at' do
        expect(subject.cache_key).to eq("#{described_class::CACHE_VERSION}:#{third_note.latest_cached_markdown_version}:#{subject.id}:#{notes_sha}:#{subject.resolved_at}")
      end
    end
  end
end
