...
RSpec.describe ProjectStatistics do
  ...
  describe '#total_repository_size' do
    it "sums repository, LFS object, and uploads size" do
      ...
      expect(statistics.total_repository_size).to eq 8
    end
  end
  ...
  describe '#update_uploads_size' do
    ...
    before do
      statistics.update_uploads_size
    end

    it 'stores the size of related uploaded files' do
      expect(statistics.uploads_size).to eq(3.megabytes)
    end
  end
