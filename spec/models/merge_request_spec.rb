...
RSpec.describe MergeRequest, factory_default: :keep do
  ...
  describe '#use_merge_base_pipeline?' do
    subject { merge_request.use_merge_base_pipeline? }

    let(:merge_request) { create(:merge_request) }

    context 'when codequality_backend_comparison is enabled' do
      before do
        stub_feature_flags(codequality_backend_comparison: true)
      end

      it { is_expected.to be_truthy }
    end

    context 'when codequality_backend_comparison is disabled' do
      before do
        stub_feature_flags(codequality_backend_comparison: false)
      end
  ...
  describe '#comparison_base' do
    subject(:pipeline) { merge_request.comparison_base(service_class) }
