...
RSpec.describe 'Issue Sidebar' do
  ...
  let!(:milestone2) { create(:milestone, project: project, due_date: 20.days.from_now) }
  let!(:milestone3) { create(:milestone, project: project, due_date: 15.days.from_now) }
  let!(:milestone4) { create(:milestone, project: project, due_date: 10.days.from_now) }
      ...
      context 'editing issue milestone', :js do
        ...
        it 'shows milestons list in the dropdown' do
          page.within('.block.milestone .dropdown-content') do
            # 4 milestones + "No milestone" = 5 items
            expect(page.find('ul')).to have_selector('li[data-milestone-id]', count: 5)
          end
        end
        ...
        it 'shows milestone due earliest at the top of the list' do
          page.within('.block.milestone .dropdown-content ul') do
            expect(page.all('li[data-milestone-id]')[1]).to have_content milestone4.title
            expect(page.all('li[data-milestone-id]')[2]).to have_content milestone3.title
            expect(page.all('li[data-milestone-id]')[3]).to have_content milestone2.title
          end
        end
      end
