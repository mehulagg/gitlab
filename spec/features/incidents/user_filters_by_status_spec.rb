...
RSpec.describe 'User filters Incident Management table by status', :js do
  ...
  let_it_be(:incident) { create(:incident, project: project, state: 'opened') }
  let_it_be(:incident) { create(:incident, project: project, state: 'opened') }
  let_it_be(:incident) { create(:incident, project: project, state: 'closed') }
  ...
  context 'when a developer+ displays the incident list they can filter the table by an incident status' do
    ...
    it 'shows the an empty table for a status with no incidents' do
