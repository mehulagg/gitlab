...
RSpec.describe LabelRowComponent, type: :component do
  ...
  context 'with a project context' do
    ...
    before do
      render_inline(described_class.new(subject: label.project, label: label))
    end

    it 'has label title' do
      expect(rendered_component).to have_text(label.title)
    end

    it 'has a non-linked label title' do
      expect(rendered_component).not_to have_link(label.title)
    end

    it 'has Issues link' do
      expect(rendered_component).to have_link('Issues')
    end

    it 'has Merge request link' do
      expect(rendered_component).to have_link('Merge requests')
    end

    it 'shows the path from where the label was created' do
      expect(rendered_component).to have_css('.label-badge', text: project.full_name)
    end
  end
  ...
  context 'with a subgroup context' do
    ...
    before do
      render_inline(described_class.new(subject: label.group, label: label))
    end
    ...
    it 'shows the path from where the label was created' do
      expect(rendered_component).to have_css('.label-badge', text: subgroup.full_name)
    end
  end
  ...
  context 'with a group context' do
    ...
    it 'does not show a path from where the label was created' do
      expect(rendered_component).not_to have_css('.label-badge')
    end
  end
  ...
  context 'with an admin context' do
    ...
    it 'does not show Issues link' do
      expect(rendered_component).not_to have_link('Issues')
    end

    it 'does not show Merge request link' do
      expect(rendered_component).not_to have_link('Merge requests')
    end
