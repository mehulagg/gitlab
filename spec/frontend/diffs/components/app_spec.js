...
describe('diffs/components/app', () => {
  ...
  describe('jumpToFile', () => {
    ...
    it('jumps to next and previous files in the list', async () => {
      ...
      expect(spy.mock.calls[spy.mock.calls.length - 1]).toEqual(['diffs/scrollToFile', { path: '222.js' }]);
      store.state.diffs.currentDiffFileId = '222';
      wrapper.vm.jumpToFile(+1);

      expect(spy.mock.calls[spy.mock.calls.length - 1]).toEqual(['diffs/scrollToFile', { path: '333.js' }]);
