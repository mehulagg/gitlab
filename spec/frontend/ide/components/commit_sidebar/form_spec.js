...
describe('IDE commit form', () => {
  ...
  // Notes:
  // - When there are no changes, there is no commit button
  describe.each`
    desc                           | stagedFiles | disabled
    ${'when there are changes'}    | ${['test']} | ${false}
    ${'when there are no changes'} | ${[]}       | ${true}
  `('$desc', ({ stagedFiles, userPermissions, disabled }) => {
    beforeEach(async () => {
  ...
  describe('on edit tab', () => {
    ...
    it('shows form when clicking commit button', async () => {
    ...
    it('toggles activity bar view when clicking commit button', async () => {
      findBeginCommitButton().vm.$emit('click');
    ...
    it('collapses if lastCommitMsg is set to empty and current view is not commit view', async () => {
      // Test that it expands when Last-Commit-Message is set
      ...
      // Now test that it collapses when Last-Commit-Message is cleared
