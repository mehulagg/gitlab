  ...
  `;
    ...
    describe('markdown preview', () => {
      ...
      it('clicking already active write or preview link does nothing', () => {
        ...
        return subject.vm
          .$nextTick()
          .then(() => assertMarkdownTabs(true, writeLink, previewLink, subject))
          .then(() => writeLink.trigger('click'))
          .then(() => subject.vm.$nextTick())
          .then(() => assertMarkdownTabs(true, writeLink, previewLink, subject))
          .then(() => previewLink.trigger('click'))
          .then(() => subject.vm.$nextTick())
          .then(() => assertMarkdownTabs(false, writeLink, previewLink, subject))
          .then(() => previewLink.trigger('click'))
          .then(() => subject.vm.$nextTick())
          .then(() => assertMarkdownTabs(false, writeLink, previewLink, subject));
      });
    ...
    it('should render attach a file button', () => {
      expect(getAttachButton().text()).toEqual('Attach a file');
    });
