...
describe('IssuableEditForm', () => {
  ...
  describe('methods', () => {
    ...
    describe('handleSaveIssuable', () => {
      it('emits `save-issuable` event on component', () => {
        const { title, description } = mockIssuable;
        wrapper.vm.handleSaveIssuable();

        expect(wrapper.emitted('save-issuable')).toBeTruthy();
        expect(wrapper.emitted('save-issuable')[0]).toEqual([
          {
            title,
            description,
          },
