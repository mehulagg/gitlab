...
describe('IssuableHeader', () => {
  ...
  describe('computed', () => {
    describe('createdAtText', () => {
      it('returns string representing value of `createAt` prop', () => {
        expect(wrapper.vm.createdAtText).toContain('Opened');
        expect(wrapper.vm.createdAtText).toContain('ago');
      });
    });
  ...
  describe('handleRightSidebarToggleClick', () => {
    ...
    it('dispatches `click` event on sidebar toggle button', () => {
      const toggleButtonEl = document.querySelector('.js-toggle-right-sidebar-button');
      jest.spyOn(toggleButtonEl, 'dispatchEvent').mockImplementation(jest.fn);

      wrapper.vm.handleRightSidebarToggleClick();

      expect(toggleButtonEl.dispatchEvent).toHaveBeenCalledWith(
  ...
  describe('template', () => {
    ...
    it('renders issuable author avatar', () => {
      ...
      const avatarUsernameEl = findByTestId('avatar-username');
      ...
      expect(avatarUsernameEl.exists()).toBe(true);
      expect(avatarUsernameEl.attributes()).toMatchObject(avatarElAttrs);
      expect(avatarUsernameEl.find('strong').text()).toBe(`@${username}`);
    });
