...
describe('the graph view selector component', () => {
  ...
  describe('events', () => {
    ...
    it('shows loading state and emits updateShowLinks when show links toggle is clicked', async () => {
      expect(wrapper.emitted().updateShowLinks).toBeUndefined();
      ...
      */
      ...
      expect(wrapper.emitted().updateShowLinks).toHaveLength(1);
      expect(wrapper.emitted().updateShowLinks).toEqual([[true]]);
    });
  });
});
