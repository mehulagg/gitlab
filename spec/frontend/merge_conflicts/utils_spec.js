...
describe('merge conflicts utils', () => {
  ...
  describe('checkLineLengths', () => {
    it('add empty lines to the left when right has more lines', () => {
      ...
      expect(result.left).toEqual([1, { lineType: 'emptyLine', richText: '' }]);
    });
    ...
    it('add empty lines to the right when left has more lines', () => {
      ...
      expect(result.right).toEqual([1, { lineType: 'emptyLine', richText: '' }]);
    });
  });

  describe('getHeadHeaderLine', () => {
    it('decorates the id', () => {
      expect(utils.getHeadHeaderLine(1)).toEqual({
  ...
  describe('decorateLineForInlineView', () => {
    ...
    `(
      ...
      ({ type, truthyProp }) => {
        expect(utils.decorateLineForInlineView({ type, rich_text: 'rich' }, 1, true)).toEqual({
          id: 1,
          hasConflict: true,
          isHead: false,
          isOrigin: false,
  ...
  describe('getOriginHeaderLine', () => {
    it('decorates the id', () => {
      expect(utils.getOriginHeaderLine(1)).toEqual({
