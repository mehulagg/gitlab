    ...
    context 'with references in pipeline variables' do
      ...
      context 'when FF :variable_inside_variable is disabled' do
        ...
        it 'returns non-expanded variables' do
          is_expected.to eq [
                              { key: 'A', value: 'refA-$B', public: false, masked: false, depends_on: %w(B) },
                              { key: 'B', value: 'refB-$C', public: false, masked: false, depends_on: %w(C) },
      ...
      context 'when FF :variable_inside_variable is enabled' do
        ...
        it 'returns expanded and sorted variables' do
          is_expected.to eq [
                              { key: 'C', value: 'value', public: false, masked: false },
                              { key: 'B', value: 'refB-value', public: false, masked: false, depends_on: %w(C) },
                              { key: 'A', value: 'refA-refB-value', public: false, masked: false, depends_on: %w(B) }
                            ]
        end
      end
    end
