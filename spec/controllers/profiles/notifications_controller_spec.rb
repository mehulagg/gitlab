...
RSpec.describe Profiles::NotificationsController do
  ...
  describe 'POST update' do
    it 'updates only permitted attributes' do
      ...
      put :update, params: { user: { notification_email: 'new@example.com', email_opted_in: false, notified_of_own_activity: true, admin: true } }

      user.reload
      expect(user.notification_email).to eq('new@example.com')
      expect(user.email_opted_in).to eq(false)
