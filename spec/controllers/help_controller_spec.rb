  ...
  describe 'GET #show' do
    context 'for Markdown formats' do
      ...
      context 'when a custom help_page_documentation_url is set in database' do
        before do
          stub_application_setting(help_page_documentation_base_url: 'https://docs.gitlab.com')
        end
      ...
      context 'when a custom help_page_documentation_url is set in configuration file' do
        let(:host) { 'https://docs.example.com' }
        let(:docs_enabled) { true }

        before do
          allow(Settings.gitlab_docs).to receive(:enabled) { docs_enabled }
          allow(Settings.gitlab_docs).to receive(:host) { host }
        end

        it_behaves_like 'documentation pages redirect', 'https://docs.example.com'
      ...
      context 'when help_page_documentation_url is set in both db and configuration file' do
        before do
          stub_application_setting(help_page_documentation_base_url: 'https://docs.gitlab.com')
          allow(Settings.gitlab_docs).to receive(:enabled) { true }
          allow(Settings.gitlab_docs).to receive(:host) { 'https://docs.example.com' }
        end
