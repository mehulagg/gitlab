  ...
  describe '.propagate' do
    ...
    context 'for a group-level integration' do
      ...
      context 'with a subgroup with integration' do
        let(:subgroup) { create(:group, parent: group) }
        let(:subgroup_integration) { create(:jira_service, group: subgroup, project: nil) }

        it 'calls to PropagateIntegrationDescendantWorker' do
          expect(PropagateIntegrationDescendantWorker).to receive(:perform_async)
