...
RSpec.describe MergeRequests::CloseService do
  ...
  describe '#execute' do
    ...
    it 'updates metrics' do
      ...
      described_class.new(container: project, current_user: user).execute(merge_request)
    end
    ...
    it 'refreshes the number of open merge requests for a valid MR', :use_clean_rails_memory_store_caching do
      service = described_class.new(container: project, current_user: user)
    ...
    context 'current user is not authorized to close merge request' do
      before do
        perform_enqueued_jobs do
          @merge_request = described_class.new(container: project, current_user: guest).execute(merge_request)
        end
      end
