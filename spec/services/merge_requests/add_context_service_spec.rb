...
RSpec.describe MergeRequests::AddContextService do
  ...
  subject(:service) { described_class.new(container: project, current_user: admin, params: { merge_request: merge_request, commits: commits }) }
  ...
  describe "#execute" do
    ...
    context "when user doesn't have permission to update merge request" do
      ...
      subject(:service) { described_class.new(container: project, current_user: user, params: { merge_request: merge_request, commits: commits }) }
    ...
    context "when the commits array is empty" do
      subject(:service) { described_class.new(container: project, current_user: admin, params: { merge_request: merge_request, commits: [] }) }
