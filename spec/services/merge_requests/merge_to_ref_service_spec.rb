  ...
  describe '#execute' do
    ...
    context 'commit history comparison with regular MergeService' do
      ...
      let(:merge_ref_service) do
        described_class.new(container: project, current_user: user)
      end

      let(:merge_service) do
        MergeRequests::MergeService.new(container: project, current_user: user, params: { sha: merge_request.diff_head_sha })
      end
