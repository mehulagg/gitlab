...
RSpec.describe MergeRequests::SquashService do
  ...
  let(:service) { described_class.new(container: project, current_user: user, params: { merge_request: merge_request }) }
  ...
  shared_examples 'the squash succeeds' do
    ...
    context 'when there is a single commit in the merge request' do
      ...
      it 'will still perform the squash when a custom squash commit message has been provided' do
        service = described_class.new(container: project, current_user: user, params: { merge_request: merge_request, squash_commit_message: 'A custom commit message' })
    ...
    context 'the squashed commit' do
      ...
      context 'if a message was provided' do
        let(:service) { described_class.new(container: project, current_user: user, params: { merge_request: merge_request, squash_commit_message: message }) }
