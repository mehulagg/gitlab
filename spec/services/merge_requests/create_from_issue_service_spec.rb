...
RSpec.describe MergeRequests::CreateFromIssueService do
  ...
  let(:service) { described_class.new(container: project, current_user: user, mr_params: service_params) }
  let(:service_with_custom_source_branch) { described_class.new(container: project, current_user: user, mr_params: { branch_name: custom_source_branch, **service_params }) }
  ...
  describe '#execute' do
    shared_examples_for 'a service that creates a merge request from an issue' do
      it 'returns an error when user can not create merge request on target project' do
        result = described_class.new(container: project, current_user: create(:user), mr_params: service_params).execute
      ...
      it 'returns an error with invalid issue iid' do
        result = described_class.new(container: project, current_user: user, mr_params: { issue_iid: -1 }).execute
      ...
      context 'when ref branch is set', :sidekiq_might_not_need_inline do
        subject { described_class.new(container: project, current_user: user, mr_params: { ref: 'feature', **service_params }).execute }
        ...
        context 'when the ref is a tag' do
          subject { described_class.new(container: project, current_user: user, mr_params: { ref: 'v1.0.0', **service_params }).execute }
        ...
        context 'when ref branch does not exist' do
          subject { described_class.new(container: project, current_user: user, mr_params: { ref: 'no-such-branch', **service_params }).execute }
