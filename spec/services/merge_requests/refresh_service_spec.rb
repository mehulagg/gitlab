...
RSpec.describe MergeRequests::RefreshService do
  ...
  describe '#execute' do
    ...
    context 'push to origin repo source branch' do
      let(:refresh_service) { service.new(container: @project, current_user: @user) }
    ...
    context 'when pipeline exists for the source branch' do
      ...
      subject { service.new(container: @project, current_user: @user).execute(@oldrev, @newrev, 'refs/heads/master') }
    ...
    describe 'Pipelines for merge requests' do
      ...
      subject { service.new(container: project, current_user: @user).execute(@oldrev, @newrev, ref) }
      ...
      context "when .gitlab-ci.yml has merge_requests keywords" do
        ...
        context "when MergeRequestUpdateWorker is retried by an exception" do
          it 'does not re-create a duplicate detached merge request pipeline' do
            expect do
              service.new(container: @project, current_user: @user).execute(@oldrev, @newrev, 'refs/heads/master')
    ...
    context 'push to origin repo target branch', :sidekiq_might_not_need_inline do
      context 'when all MRs to the target branch had diffs' do
        before do
          service.new(container: @project, current_user: @user).execute(@oldrev, @newrev, 'refs/heads/feature')
          reload_mrs
        end
      ...
      context 'manual merge of source branch', :sidekiq_might_not_need_inline do
        before do
          ...
          service.new(container: @project, current_user: @user).execute(@oldrev, commit.id, 'refs/heads/feature')
          reload_mrs
        end
    ...
    context 'push to fork repo source branch', :sidekiq_might_not_need_inline do
      let(:refresh_service) { service.new(container: @fork_project, current_user: @user) }
    ...
    context 'push to fork repo target branch', :sidekiq_might_not_need_inline do
      describe 'changes to merge requests' do
        before do
          service.new(container: @fork_project, current_user: @user).execute(@oldrev, @newrev, 'refs/heads/feature')
          reload_mrs
        end
    ...
    context 'forked projects with the same source branch name as target branch', :sidekiq_might_not_need_inline do
      ...
      it 'reloads a new diff for a push to the forked project' do
        expect do
          service.new(container: @fork_project, current_user: @user).execute(@oldrev, first_commit, 'refs/heads/master')
          reload_mrs
      ...
      it 'reloads a new diff for a force push to the source branch' do
        expect do
          service.new(container: @fork_project, current_user: @user).execute(@oldrev, force_push_commit, 'refs/heads/master')
          reload_mrs
        end.to change { forked_master_mr.merge_request_diffs.count }.by(1)
      end
      ...
      it 'reloads a new diff for a force push to the target branch' do
        expect do
          service.new(container: @project, current_user: @user).execute(@oldrev, force_push_commit, 'refs/heads/master')
          reload_mrs
        end.to change { forked_master_mr.merge_request_diffs.count }.by(1)
      end
      ...
      it 'reloads a new diff for a push to the target project that contains a commit in the MR' do
        expect do
          service.new(container: @project, current_user: @user).execute(@oldrev, first_commit, 'refs/heads/master')
          reload_mrs
        end.to change { forked_master_mr.merge_request_diffs.count }.by(1)
      end
      ...
      it 'does not increase the diff count for a new push to target branch' do
        ...
        expect do
          service.new(container: @project, current_user: @user).execute(@newrev, new_commit, 'refs/heads/master')
          reload_mrs
    ...
    context 'merge request metrics' do
      ...
      context 'when the merge request is sourced from the same project' do
        it 'creates a `MergeRequestsClosingIssues` record for each issue closed by a commit' do
          ...
          refresh_service = service.new(container: project, current_user: user)
          allow(refresh_service).to receive(:execute_hooks)
      ...
      context 'when the merge request is sourced from a different project' do
        it 'creates a `MergeRequestsClosingIssues` record for each issue closed by a commit' do
          ...
          refresh_service = service.new(container: forked_project, current_user: user)
          allow(refresh_service).to receive(:execute_hooks)
          refresh_service.execute(@oldrev, @newrev, 'refs/heads/close-by-commit')
