...
RSpec.describe Namespaces::InProductMarketingEmailsService, '#execute' do
  ...
  before do
    ...
    allow(Notify).to receive(:in_product_marketing_email).and_call_original
  end

  RSpec::Matchers.define :send_in_product_marketing_email do |*args|
    match do
      expect(Notify).to have_received(:in_product_marketing_email).with(*args).exactly(1)
    end
  ...
  describe 'experimentation' do
    context 'when the experiment is enabled' do
      it 'adds the group as an experiment subject in the experimental group' do
        expect(Experiment).to receive(:add_group).with(:in_product_marketing_emails, :experimental, group)
        execute_service
      end
    end
    ...
    context 'when the experiment is disabled' do
      ...
      it 'adds the group as an experiment subject in the control group' do
        expect(Experiment).to receive(:add_group).with(:in_product_marketing_emails, :control, group)
        execute_service
      end
