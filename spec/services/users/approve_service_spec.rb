    ...
    context 'success' do
      context 'when the executor user is an admin in admin mode', :enable_admin_mode do
        ...
        it 'logs approval in application logs' do
          ...
          expect(Gitlab::AppLogger).to have_received(:info).with("USER INSTANCE REQUEST APPROVED: user: #{user.username}, email: #{user.email}, approved_by: #{current_user.username}, ip_address=#{current_user.current_sign_in_ip}")
        end
