...
RSpec.describe Clusters::Applications::UninstallService, '#execute' do
  ...
  context 'when there are no errors' do
    before do
      expect(helm_client).to receive(:uninstall).with(kind_of(Gitlab::Kubernetes::Helm::V2::DeleteCommand))
  ...
  context 'when k8s cluster communication fails' do
    ...
    before do
      expect(helm_client).to receive(:uninstall).with(kind_of(Gitlab::Kubernetes::Helm::V2::DeleteCommand)).and_raise(error)
    end
