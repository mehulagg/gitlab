...
RSpec.describe Backup::GitalyRpcBackup do
  let(:progress) { spy(:stdin) }

  subject { described_class.new(progress) }

  context 'unknown' do
    it 'fails to start unknown' do
      expect { subject.start(:unknown) }.to raise_error(::Backup::Error, 'unknown backup type')
    end
  end
  ...
  context 'restore' do
    ...
    it 'restores from repository bundles', :aggregate_failures do
      FileUtils.mkdir_p(File.join(Gitlab.config.backup.path, 'repositories', project.disk_path))
      FileUtils.mkdir_p(File.join(Gitlab.config.backup.path, 'repositories', personal_snippet.disk_path))
      FileUtils.mkdir_p(File.join(Gitlab.config.backup.path, 'repositories', project_snippet.disk_path))
      FileUtils.cp(Rails.root.join('spec/fixtures/lib/backup/project_repo.bundle'), File.join(Gitlab.config.backup.path, 'repositories', project.disk_path + '.bundle'))
      FileUtils.cp(Rails.root.join('spec/fixtures/lib/backup/wiki_repo.bundle'), File.join(Gitlab.config.backup.path, 'repositories', project.disk_path + '.wiki.bundle'))
      FileUtils.cp(Rails.root.join('spec/fixtures/lib/backup/design_repo.bundle'), File.join(Gitlab.config.backup.path, 'repositories', project.disk_path + '.design.bundle'))
      FileUtils.cp(Rails.root.join('spec/fixtures/lib/backup/personal_snippet_repo.bundle'), File.join(Gitlab.config.backup.path, 'repositories', personal_snippet.disk_path + '.bundle'))
      FileUtils.cp(Rails.root.join('spec/fixtures/lib/backup/project_snippet_repo.bundle'), File.join(Gitlab.config.backup.path, 'repositories', project_snippet.disk_path + '.bundle'))
