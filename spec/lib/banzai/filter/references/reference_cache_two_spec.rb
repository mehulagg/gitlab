...
RSpec.describe Banzai::Filter::References::ReferenceCacheTwo do
  ...
  let(:parent) { project }
  let(:cache) { described_class.new(filter_class.object_class, parent_type, parent, filter, { project: project })  }
  ...
  describe '#load_references_per_parent' do
    ...
    it 'returns a Hash containing references grouped per parent paths' do
      refs = cache.load_references_per_parent(filter.nodes)
  ...
  describe '#parent_per_reference' do
    ...
    it 'returns a Hash containing projects grouped per parent paths' do
      ...
      expect(cache.parent_per_reference).to eq({ project.full_path => project })
    end
  end
