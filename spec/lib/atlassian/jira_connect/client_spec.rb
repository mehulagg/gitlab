  ...
  describe '#store_build_info' do
    ...
    let_it_be(:pipelines) do
      (red_herrings + mrs_by_branch + mrs_by_title).map do |mr|
        create(:ci_pipeline, head_pipeline_of: mr)
      end
    end
    ...
    it "calls the API with auth headers" do
      subject.store_build_info(project: project, pipelines: pipelines)
    end
    ...
    it 'does not call the api if there is nothing to report' do
      ...
      subject.store_build_info(project: project, pipelines: pipelines.take(1))
    end
    ...
    it 'avoids N+1 database queries' do
      ...
      expect { subject.store_build_info(project: project, pipelines: pipelines) }.not_to exceed_query_limit(baseline)
    end
  end
