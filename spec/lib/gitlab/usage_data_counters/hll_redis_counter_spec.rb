...
RSpec.describe Gitlab::UsageDataCounters::HLLRedisCounter, :clean_gitlab_redis_shared_state do
  ...
  describe 'known_events' do
    ...
    describe '.unique_events' do
      ...
      it 'returns 0 if there are no events and does not call HLL count' do
        allow(Gitlab::UsageDataCounters::HLLRedisCounter).to receive(:keys_for_aggregation).and_return([])

        expect(Gitlab::Redis::HLL).not_to receive(:count)
        expect(described_class.unique_events(event_names: [weekly_event], start_date: 4.weeks.ago, end_date: Date.current)).to eq(0)
      end
  ...
  describe '.calculate_events_union' do
    ...
    it 'returns 0 if there are no events and does not call HLL count' do
      ...
      expect(Gitlab::Redis::HLL).not_to receive(:count)
      expect(described_class.calculate_events_union(**time_range.merge(event_names: %w[event1_slot event2_slot event3_slot]))).to eq 0
    end
  end
