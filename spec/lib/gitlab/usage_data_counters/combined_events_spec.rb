...
RSpec.describe 'combined events' do
  ...
  YAML.load_file(Rails.root.join(Gitlab::UsageDataCounters::HLLRedisCounter::COMBINED_EVENTS_PATH))&.map(&:with_indifferent_access).tap do |combined_events|
    it 'all events has unique name' do
      event_names = combined_events&.map { |event| event[:name] }

      expect(event_names).to eq(event_names&.uniq)
    end

    combined_events&.each do |aggregate|
      context "for #{aggregate[:name]} aggregate of #{aggregate[:events].join(' ')}" do
        ...
        it "uses allowed aggregation operators" do
          expect(Gitlab::UsageDataCounters::HLLRedisCounter::ALLOWED_COMBINED_EVENTS_AGGREGATIONS).to include aggregate[:operator]
        end
