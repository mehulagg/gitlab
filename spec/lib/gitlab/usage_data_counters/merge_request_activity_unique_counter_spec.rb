...
RSpec.describe Gitlab::UsageDataCounters::MergeRequestActivityUniqueCounter, :clean_gitlab_redis_shared_state do
  ...
  shared_examples_for 'a not tracked merge request unique event' do
    specify do
      expect(Gitlab::UsageDataCounters::HLLRedisCounter).not_to receive(:unique_events)
  ...
  describe '.track_mr_including_ci_config' do
    ...
    context 'when merge request includes a ci config change' do
      before do
        allow(merge_request).to receive(:new_paths).and_return(%w(abc.txt .gitlab-ci.yml))
      end
      ...
      context 'when FF usage_data_o_pipeline_authoring_unique_users_pushing_mr_ciconfigfile is disabled' do
        ...
        it_behaves_like 'a not tracked merge request unique event'
      end
    end

    context 'when merge request does not include any ci config change' do
      before do
        allow(merge_request).to receive(:new_paths).and_return(%w(abc.txt abc.xyz))
      end
