...
RSpec.describe 'aggregated metrics' do
  ...
  Gitlab::ProductIntelligence::AggregatedMetrics::Aggregate.new.send(:aggregated_metrics).tap do |aggregated_metrics|
    ...
    aggregated_metrics&.each do |aggregate|
      context "for #{aggregate[:name]} aggregate of #{aggregate[:events].join(' ')}" do
        ...
        it "uses allowed aggregation operators" do
          expect(Gitlab::ProductIntelligence::AggregatedMetrics::ALLOWED_METRICS_AGGREGATIONS).to include aggregate[:operator]
        end
