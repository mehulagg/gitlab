...
RSpec.describe Gitlab::Experimentation do
  describe '.registry' do
    it 'proplery loads YAML file' do
    ...
    it 'temporarily ensures we know what experiments exist for backwards compatibility' do
      ...
      backwards_compatible_experiment_keys = described_class.registry.filter { |_, v| v[:use_backwards_compatible_subject_index] }.keys
  ...
  context 'with stubbed registry' do
    before do
      allow(Gitlab::Experimentation).to receive(:registry).and_return(
        {
