...
RSpec.describe Gitlab::Utils::UsageData do
  ...
  describe '#estimate_batch_distinct_count' do
    ...
    it 'delegates counting to counter class instance' do
      ...
      expect(described_class.estimate_batch_distinct_count(relation, 'column', batch: false)).to eq(5)
    end
    ...
    it 'returns default fallback value when counting fails due to database error' do
      ...
      expect(described_class.estimate_batch_distinct_count(relation, batch: false)).to eq(15)
    end
    ...
    context 'feature flag: postgres_hll_batch_counting is disabled' do
      before do
        stub_feature_flags(postgres_hll_batch_counting: false)
      end

      it 'returns the count when counting succeeds' do
        expect(described_class).to receive(:distinct_count)
                                     .with(relation, 'column', batch: false, batch_size: 10, start: 1, finish: 11)
                                     .and_return(5)

        expect(described_class.estimate_batch_distinct_count(relation, 'column', batch: false, batch_size: 10, start: 1, finish: 11)).to eq(5)
      end
    end
  end
