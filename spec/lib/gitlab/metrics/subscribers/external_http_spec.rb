...
RSpec.describe Gitlab::Metrics::Subscribers::ExternalHttp do
  ...
  describe '.detail_store' do
    context 'when SafeRequestStore is not activated' do
  ...
  describe '.payload' do
    ...
    context 'when SafeRequestStore does not have any item from external HTTP' do
      ...
      it 'returns an empty array' do
        expect(described_class.payload).to eql(external_http_count: 0, external_http_duration: 0.0)
      end
    end
    ...
    context 'when external HTTP recorded some values' do
      before do
        ...
        Gitlab::SafeRequestStore[:external_http_duration] = 1.2
      end
      ...
      it 'returns the external http detailed store' do
        expect(described_class.payload).to eql(external_http_count: 7, external_http_duration: 1.2)
      end
    end
  end
  ...
  describe '#request' do
    ...
    it 'tracks external HTTP exceptions' do
      expect(transaction).to receive(:increment)
        .with(:gitlab_external_http_total, 1, { code: nil, method: "POST" })
      expect(transaction).to receive(:increment)
    ...
    it 'stores per-request counters' do
      ...
      expect(Gitlab::SafeRequestStore[:external_http_duration]).to eq(5.741) # 0.321 + 0.12 + 5.3
    end
