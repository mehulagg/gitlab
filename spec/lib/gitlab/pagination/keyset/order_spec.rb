...
RSpec.describe Gitlab::Pagination::Keyset::Order do
  ...
  def iterate_and_collect(order:, page_size:, where_conditions: '1=1')
    all_items = []
    ...
    loop do
      ...
      where_conditions = order.build_conditions_recursively(order.column_definitions, cursor_attributes).to_sql
    end
  ...
  shared_examples 'order examples' do
    ...
    context 'when paginating backwards' do
      subject do
        ...
        cursor_attributes = order.cursor_attributes_for_node(last_item)
        where_conditions = order.build_conditions_recursively(order.reversed_order.column_definitions, cursor_attributes)
  ...
  context 'when ordering non nullable columns with mixed directions' do
    let(:table_data) do
      <<-SQL
      VALUES (1,  2010, 0),
             (2,  2011, 0),
