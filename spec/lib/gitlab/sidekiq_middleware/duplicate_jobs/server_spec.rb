...
RSpec.describe Gitlab::SidekiqMiddleware::DuplicateJobs::Server, :clean_gitlab_redis_queues do
  shared_examples 'server duplicate job' do |strategy|
    let(:worker_class) do
      Class.new do
        def self.name
  ...
  context 'with until_executing strategy' do
    it_behaves_like 'server duplicate job', :until_executing
  end

  context 'with until_executed strategy' do
    it_behaves_like 'server duplicate job', :until_executed
  end
end
