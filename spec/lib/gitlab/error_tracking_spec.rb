...
RSpec.describe Gitlab::ErrorTracking do
  ...
  let(:sentry_event) { Gitlab::Json.parse(Sentry.get_current_client.transport.events.last.to_json) }
  ...
  describe '.with_context' do
    it 'adds the expected tags' do
      ...
      expect(Sentry.get_current_scope.tags[:locale].to_s).to eq(I18n.locale.to_s)
      expect(Sentry.get_current_scope.tags[Labkit::Correlation::CorrelationId::LOG_KEY.to_sym].to_s)
        .to eq('cid')
    end
  end
