  ...
  describe '#execute' do
    ...
    context 'all restores are executed' do
      ...
      it 'calls RepoRestorer with project and wiki' do
        ...
        expect(Gitlab::ImportExport::RepoRestorer).to receive(:new).with(path_to_bundle: repo_path, shared: shared, exportable: project).and_return(restorer)
        expect(Gitlab::ImportExport::RepoRestorer).to receive(:new).with(path_to_bundle: wiki_repo_path, shared: shared, exportable: ProjectWiki.new(project)).and_return(restorer)
