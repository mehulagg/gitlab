  ...
  describe '#perform' do
    ...
    let!(:job1) do
      table(:background_migration_jobs).create!(
        ...
        arguments: [1, 10, table_name, 'id', 'id', 'id_convert_to_bigint']
      )
    end
    ...
    let!(:job2) do
      table(:background_migration_jobs).create!(
        class_name: migration_class,
        arguments: [11, 20, table_name, 'id', 'id', 'id_convert_to_bigint']
      )
    end

    it 'copies all primary keys in range' do
      subject.perform(12, 15, table_name, 'id', 'id', 'id_convert_to_bigint')
    ...
    it 'copies all foreign keys in range' do
      subject.perform(10, 14, table_name, 'id', 'fk', 'fk_convert_to_bigint')
    ...
    it 'copies columns with NULLs' do
      ...
      subject.perform(10, 20, table_name, 'id', 'name', 'name_convert_to_text')
    ...
    it 'tracks completion with BackgroundMigrationJob' do
      expect do
        subject.perform(11, 20, table_name, 'id', 'id', 'id_convert_to_bigint')
