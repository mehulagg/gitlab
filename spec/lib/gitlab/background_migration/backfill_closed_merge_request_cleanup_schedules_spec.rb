...
RSpec.describe Gitlab::BackgroundMigration::BackfillClosedMergeRequestCleanupSchedules, schema: 20201023114628 do
  ...
  describe '#perform' do
    ...
    let!(:merged_mr) { merge_requests.create!(target_project_id: project.id, source_branch: 'master', target_branch: 'master', state_id: '3') }
    ...
    it 'creates records for all closed merge requests in range' do
      subject.perform(open_mr.id, closed_mr_2.id)

      aggregate_failures do
        expect(cleanup_schedules.all.pluck(:merge_request_id))
          .to contain_exactly(closed_mr_1.id, closed_mr_2.id)
        expect(cleanup_schedules.find_by(merge_request_id: closed_mr_1.id).scheduled_at)
          .to eq(closed_mr_1_metrics.latest_closed_at + 14.days)
        expect(cleanup_schedules.find_by(merge_request_id: closed_mr_2.id).scheduled_at)
          .to eq(closed_mr_2_cleanup_schedule.scheduled_at)
      end
    end
  end
end
