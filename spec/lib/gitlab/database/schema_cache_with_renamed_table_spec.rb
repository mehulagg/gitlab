...
RSpec.describe Gitlab::Database::SchemaCacheWithRenamedTable do
  let(:old_model) do
    Class.new(ActiveRecord::Base) do
      ...
      table_will_be_renamed from: 'projects', to: 'projects_new'
    end
  end
  ...
  context 'when table is not renamed yet' do
    ...
    it 'uses the original table to look up metadata' do
      expect(ActiveRecord::Base.connection.schema_cache).to receive(:with_renamed_table).with('projects').and_return('projects')
  ...
  context 'when table is renamed' do
    ...
    it 'uses the renamed table to look up metadata' do
      expect(ActiveRecord::Base.connection.schema_cache).to receive(:with_renamed_table).with('projects').and_return('projects_new')
