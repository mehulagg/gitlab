    ...
    it 'raises an error when migration exists and is not marked as finished' do
      ...
      expect { ensure_batched_background_migration_is_finished }
        .to raise_error "Expected batched background migration for the given configuration to be marked as 'finished', but it is 'active': " \
          "job_class_name: #{configuration[:job_class_name]}, table_name: #{configuration[:table_name]}, column_name: #{configuration[:column_name]}, " \
          "job_arguments: #{configuration[:job_arguments].inspect}"
    end

    it 'does not raise error when migration exists and is marked as finsihed' do
