...
RSpec.describe Gitlab::Database::PostgresForeignKey, type: :model do
  ...
  describe '#by_referenced_table_identifier' do
    it 'throws an error when the identifier name is not fully qualified' do
      expect { Gitlab::Database::PostgresForeignKey.by_referenced_table_identifier('referenced_table') }.to raise_error(ArgumentError, /not fully qualified/)
    end

    it 'finds the foreign keys for the referenced table' do
      expected = Gitlab::Database::PostgresForeignKey.find_by!(name: 'fk_constrained_to_referenced')

      expect(Gitlab::Database::PostgresForeignKey.by_referenced_table_identifier('public.referenced_table')).to contain_exactly(expected)
    end
  end
end
