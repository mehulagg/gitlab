...
RSpec.describe Gitlab::Database::PostgresIndex do
  ...
  describe '#regular' do
    it 'excludes unique indexes' do
      expect(described_class.regular).to all(have_attributes(is_unique: false))
    end

    it 'excludes indexes of partitions' do
      expect(described_class.regular).to all(satisfy("not be a partitioned index") { |index| index.schema !~ /^gitlab_partitions/ })
    end
  end

  describe '#random_few' do
    it 'limits to two records by default' do
      expect(described_class.random_few.size).to eq(2)
    end
  end
