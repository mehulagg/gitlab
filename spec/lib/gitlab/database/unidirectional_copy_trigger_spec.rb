  ...
  describe '.name' do
    context 'when a single column name is given' do
      it 'returns the trigger name' do
        trigger_name = copy_trigger.name('id', 'other_id')
    ...
    context 'when multiple column names are given' do
      it 'returns the trigger name' do
        trigger_name = copy_trigger.name(%w[id fk_id], %w[other_id other_fk_id])
    ...
    context 'when a different number of new and old column names are given' do
      it 'raises an error' do
        ...
        end.to raise_error(ArgumentError, 'number of source columns must match number of destination columns')
      end
    end
  end
