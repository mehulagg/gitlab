  ...
  describe '#for' do
    ...
    context 'for add metrics' do
      it_behaves_like 'name suggestion' do
        ...
        let(:operation) { :add }
        let(:data) { { personal_snippets: described_class.for(:count, relation: PersonalSnippet), project_snippets: described_class.for(:count, relation: ProjectSnippet) } }
        let(:relation) { [data[:personal_snippets], data[:project_snippets]] }
        let(:column) { nil }
