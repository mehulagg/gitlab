...
RSpec.describe ::Gitlab::Graphql::Deprecation do
  ...
  describe '#reason_text' do
    described_class::REASONS.each_key do |known_reason|
      context "when the reason is a known reason such as #{known_reason.inspect}" do
        ...
        it 'renders the reason_text correctly' do
          expect(deprecation.reason_text).to eq(described_class::REASONS[known_reason])
        end
      end
    end
    ...
    context 'when the reason is any other string' do
      ...
      it 'appends a period' do
        expect(deprecation.reason_text).to eq('unhelpful.')
      end
    end
  end
  ...
  describe '#description_suffix' do
    ...
    it 'renders as milestone:reason with a leading space' do
      expect(deprecation.description_suffix).to eq(' Deprecated in 10.10: This was renamed.')
    end
  end
