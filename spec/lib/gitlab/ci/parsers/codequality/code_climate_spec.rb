...
RSpec.describe Gitlab::Ci::Parsers::Codequality::CodeClimate do
  describe '#parse!' do
    subject { described_class.new.parse!(code_climate, codequality_report) }
    ...
    context "when data is code_climate style JSON" do
      context "when there are no degredations" do
      ...
      context "when there are degredations" do
        it "returns a codequality report" do
          expect { subject }.not_to raise_error

          expect(codequality_report.degredations_count).to eq(1)
        end
      end
    end
