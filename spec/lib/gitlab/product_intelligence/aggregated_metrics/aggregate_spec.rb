...
RSpec.describe Gitlab::ProductIntelligence::AggregatedMetrics::Aggregate, :clean_gitlab_redis_shared_state do
  ...
  context 'aggregated_metrics_data' do
    ...
    shared_examples 'aggregated_metrics_data' do
      context 'no aggregated metrics is defined' do
        it 'returns empty hash' do
          allow_any_instance_of(described_class).to receive(:aggregated_metrics).and_return([])
      ...
      context 'there are aggregated metrics defined' do
        before do
          allow_any_instance_of(described_class).to receive(:aggregated_metrics).and_return(aggregated_metrics)
        end
    ...
    describe '.aggregated_metrics_weekly_data' do
      subject(:aggregated_metrics_data) { described_class.new.aggregated_metrics_weekly_data }
    ...
    describe '.aggregated_metrics_monthly_data' do
      subject(:aggregated_metrics_data) { described_class.new.aggregated_metrics_monthly_data }
        ...
        it 'caches intermediate operations' do
          ...
          4.downto(1) do |subset_size|
            known_events.combination(subset_size).each do |events|
              keys = Gitlab::UsageDataCounters::HLLRedisCounter.send(:weekly_redis_keys, events: events, start_date: 4.weeks.ago.to_date, end_date: Date.current)
