...
RSpec.describe Security::CiConfiguration::SastBuildAction do
  ...
  describe 'Security::CiConfiguration::SastBuildAction::SAST_ANALYZERS' do
    subject(:variable) {Security::CiConfiguration::SastBuildAction::SAST_ANALYZERS}

    it 'is sorted alphabetically' do
      sorted_variable = Security::CiConfiguration::SastBuildAction::SAST_ANALYZERS
        .split(',')
        .map(&:strip)
        .sort
        .join(', ')
