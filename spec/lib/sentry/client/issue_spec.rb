...
RSpec.describe GitlabSentry::Client::Issue do
  include SentryClientHelpers

  let(:token) { 'test-token' }
  let(:sentry_url) { 'https://sentrytest.gitlab.com/api/0' }
  let(:client) { GitlabSentry::Client.new(sentry_url, token) }
  let(:issue_id) { 11 }
  ...
  describe '#list_issues' do
    ...
    context 'with invalid sort params' do
      ...
      it 'throws an error' do
        expect { subject }.to raise_error(GitlabSentry::Client::BadRequestError, 'Invalid value for sort param')
      end
    end
    ...
    context 'essential keys missing in API response' do
      ...
      it 'raises exception' do
        expect { subject }.to raise_error(GitlabSentry::Client::MissingKeysError, 'Sentry API response is missing keys. key not found: "id"')
      end
    end
    ...
    context 'sentry api response too large' do
      it 'raises exception' do
        ...
        expect { subject }.to raise_error(GitlabSentry::Client::ResponseInvalidSizeError, 'Sentry API response is too big. Limit is 1 MB.')
      end
    end
