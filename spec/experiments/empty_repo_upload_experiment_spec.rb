...
RSpec.describe EmptyRepoUploadExperiment, :experiment do
  ...
  describe '#track_initial_write' do
    it "tracks an event for the first commit on a project" do
      allow(Gitlab::ProjectCommitCount).to receive(:commit_count_for).and_return(1)
    ...
    it "doesn't track an event for projects with a commit count more than 1" do
      allow(Gitlab::ProjectCommitCount).to receive(:commit_count_for).and_return(2)
    ...
    it "handles exceptions by logging them" do
      allow(Gitlab::ProjectCommitCount).to receive(:commit_count_for).and_raise(e = StandardError.new('_message_'))

      expect(Gitlab::ErrorTracking).to receive(:track_exception).with(e, experiment: 'empty_repo_upload')
