...
RSpec.describe 'layouts/nav/sidebar/_project' do
  ...
  shared_examples 'shows menus' do
    it 'renders scope menu' do
      render

      expect(rendered).to have_css(".context-header a[href='/#{project.full_path}'] .avatar-container")
      expect(rendered).to have_css(".context-header a .sidebar-context-title")
    end
  end

  it_behaves_like 'has nav sidebar'
  it_behaves_like 'sidebar includes snowplow attributes', 'render', 'projects_side_navigation', 'projects_side_navigation'
  it_behaves_like 'shows menus'

  context 'when feature flag :project_source_code_refactor is disabled' do
    before do
