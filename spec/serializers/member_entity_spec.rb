...
RSpec.describe MemberEntity do
  ...
  shared_examples 'invite' do
    it 'correctly sets `invite.avatar_url` property' do
      ...
      expect(entity_hash[:invite][:avatar_url]).to be(avatar_url)
    end

    it 'correctly sets `invite.can_resend` property' do
