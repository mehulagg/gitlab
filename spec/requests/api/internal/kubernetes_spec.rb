  ...
  describe 'POST /internal/kubernetes/usage_metrics' do
    ...
    context 'is authenticated for an agent' do
      ...
      it 'returns no_content for valid events' do
        send_request(params: { gitops_sync_count: 10, proxy_request_count: 5 })

        expect(response).to have_gitlab_http_status(:no_content)
      end

      it 'returns no_content for counts of zero' do
        send_request(params: { gitops_sync_count: 0, proxy_request_count: 0 })

        expect(response).to have_gitlab_http_status(:no_content)
      end

      it 'returns 400 for non number' do
        send_request(params: { gitops_sync_count: 'string', proxy_request_count: 1 })

        expect(response).to have_gitlab_http_status(:bad_request)
      end

      it 'returns 400 for negative number' do
        send_request(params: { gitops_sync_count: -1, proxy_request_count: 1 })
