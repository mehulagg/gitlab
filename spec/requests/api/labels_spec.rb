  ...
  shared_examples 'label update API' do
    route_types.each do |route_type|
      ...
      it "returns 400 if no new parameters given (#{route_type} route)" do
        ...
        expect(json_response['error']).to eq('new_name, color, description, priority, remove_on_issue_close are missing, '\
      ...
      it "returns 200 if remove_on_issue_close is changed (#{route_type} route)" do
        put_labels_api(route_type, user, spec_params, remove_on_issue_close: true)

        expect(response).to have_gitlab_http_status(:ok)
        expect(json_response['id']).to eq(expected_response_label_id)
        expect(json_response['remove_on_issue_close']).to eq(true)
      end
    end
