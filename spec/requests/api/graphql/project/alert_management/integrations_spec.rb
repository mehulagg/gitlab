  ...
  context 'with integrations' do
    ...
    context 'with project permissions' do
      ...
      context 'when HTTP Integration ID is given' do
        let(:params) { { id: GitlabSchema.id_from_object(active_http_integration).to_s } }

        it_behaves_like 'a working graphql query'

        it { expect(integrations.size).to eq(1) }
      ...
      context 'when Prometheus Integration ID is given' do
        let(:params) { { id: GitlabSchema.id_from_object(prometheus_service).to_s } }
