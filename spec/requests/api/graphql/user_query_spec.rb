...
RSpec.describe 'getting user information' do
  ...
  context 'looking up a user by username' do
    ...
    let(:reviewed_mrs) { graphql_data_at(:user, :reviewed_merge_requests, :nodes) }
    ...
    context 'the user is an active user' do
      ...
      describe 'reviewedMergeRequests' do
        let(:user_fields) do
          query_graphql_field(:reviewed_merge_requests, mr_args, 'nodes { id }')
        end
