...
RSpec.describe 'Query.project.pipeline' do
  ...
  describe '.stages.groups.jobs' do
    ...
    describe 'performance' do
      before do
        ...
        create(:ci_build, pipeline: pipeline, stage_id: build_stage.id, name: 'docker 1 2')
        create(:ci_build, pipeline: pipeline, stage_id: build_stage.id, name: 'docker 2 2')
        create(:ci_bridge, pipeline: pipeline, stage_id: test_stage.id, name: 'rspec 1 2')
        create(:ci_bridge, pipeline: pipeline, stage_id: test_stage.id, name: 'rspec 2 2')
      end
  ...
  describe '.jobs.artifacts' do
    ...
    it 'does not generate N+1 queries', :request_store, :use_sql_query_cache do
      create(:ci_build, name: 'test', pipeline: pipeline)
      post_graphql(query, current_user: user)
