...
RSpec.describe QA::Specs::Runner do
  ...
  describe '#perform' do
    ...
    context 'when "--tag smoke" is set as options' do
      ...
      it 'focuses on the given tag without excluding the orchestrated tag' do
        expect_rspec_runner_arguments(['--tag', 'smoke', '--tag', '~geo', *described_class::DEFAULT_TEST_PATH_ARGS])
    ...
    context 'when "--tag smoke" and "qa/specs/features/foo" are set as options' do
      ...
      it 'focuses on the given tag and includes the path without excluding the orchestrated tag' do
        expect_rspec_runner_arguments(['--tag', 'smoke', '--tag', '~geo', 'qa/specs/features/foo'])
    ...
    context 'when "--tag geo" is set as options' do
      subject { described_class.new.tap { |runner| runner.options = %w[--tag geo] } }

      it 'focuses on the geo tag without excluding the orchestrated tag' do
