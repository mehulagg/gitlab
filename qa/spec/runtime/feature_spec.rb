  ...
  shared_examples 'enables a feature flag' do
    it 'enables a feature flag for a scope' do
      allow(described_class).to receive(:get)
        .and_return(Struct.new(:code, :body).new(200, '[{ "name": "a-flag", "state": "on" }]'))
    ...
    context 'when the flag is enabled for a scope' do
      it 'returns the feature flag state' do
        ...
        expect(described_class)
          .to receive(:get)
          .and_return(Struct.new(:code, :body).new(200, %Q([{ "name": "a-flag", "state": "conditional", "gates": #{gates} }])))

        expect(described_class.enabled?('a-flag', scope => actor)).to be_truthy
      end
    end
  end
