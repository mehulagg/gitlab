...
module QA
  module Resource
    class RegistryRepository < Base
      ...
      def remove_via_api!
        registry_repositories = project.container_repositories
        if registry_repositories && !registry_repositories.empty?
          this_registry_repository = registry_repositories.find { |registry_repository| registry_repositories[:name] == name }
