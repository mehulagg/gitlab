...
module QA
  RSpec.describe 'Verify', :runner do
    describe 'Pipeline with masked variable' do
      let(:executor) { "qa-runner-#{Faker::Alphanumeric.alphanumeric(8)}" }
      let(:protected_value) { Faker::Alphanumeric.alphanumeric(8) }

      let(:project) do
        Resource::Project.fabricate_via_api! do |project|
          project.name = 'project-with-masked-variable'
          project.description = 'project with masked variable'
        end
      end
      ...
      it 'masks variable on protected branch', testcase: 'https://gitlab.com/gitlab-org/quality/testcases/-/issues/157' do
        ...
        [developer, maintainer].each do |user|
          ...
          Page::Project::Job::Show.perform do |show|
            expect(show.output).to have_no_content(masked_value), 'Expect masked variable to be [MASKED].'
          end
        end
      end

      it 'masks variable on unprotected branch', testcase: 'https://gitlab.com/gitlab-org/quality/testcases/-/issues/157' do
        [developer, maintainer].each do |user|
          create_merge_request(Runtime::API::Client.new(:gitlab, user: user))
          go_to_pipeline_job(user)

          Page::Project::Job::Show.perform do |show|
            expect(show.output).to have_no_content(protected_value), 'Expect masked variable to be [MASKED].'
          end
        end
      end
      ...
      def add_masked_variable
        Resource::CiVariable.fabricate_via_api! do |masked_variable|
          masked_variable.project = project
          masked_variable.key = 'MASKED_VARIABLE'
          masked_variable.value = masked_value
          masked_variable.masked = true
        end
      end
