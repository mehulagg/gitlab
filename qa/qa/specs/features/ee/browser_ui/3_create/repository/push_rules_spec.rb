    ...
    context 'Push Rules' do
      describe 'using non signed commits' do
        before(:context) do
          ...
          @branch_name_limitation = Runtime::Env.default_branch
        ...
        it 'restricts removal of tag', testcase: 'https://gitlab.com/gitlab-org/quality/testcases/-/issues/650' do
          tag = Resource::Tag.fabricate_via_api! do |tag|
            tag.project = @project
            tag.ref = Runtime::Env.default_branch
      ...
      def push(commit_message:, branch:, file:, user:, tag:, gpg:)
        Resource::Repository::ProjectPush.fabricate! do |push|
          ...
          push.new_branch = branch != Runtime::Env.default_branch
      ...
      def expect_no_error_on_push(commit_message: 'allowed commit', branch: Runtime::Env.default_branch, file:, user: @creator, tag: nil, gpg: nil)
        expect do
          push commit_message: commit_message, branch: branch, file: file, user: user, tag: tag, gpg: gpg
        end.not_to raise_error
      end

      def expect_error_on_push(commit_message: 'allowed commit', branch: Runtime::Env.default_branch, file:, user: @creator, tag: nil, gpg: nil, error: nil)
        expect do
          push commit_message: commit_message, branch: branch, file: file, user: user, tag: tag, gpg: gpg
